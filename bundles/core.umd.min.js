...
err.stack=context.source,context.view.state|=ViewState.Errored,err}function isViewDebugError(err){return!!getDebugContext(err)}function viewDestroyedError$1(action){return new Error("ViewDestroyedError: Attempt to use a destroyed view: "+action)}function tokenKey(token){var key=_tokenKeyCache.get(token);return key||(key=stringify(token)+"_"+_tokenKeyCache.size,_tokenKeyCache.set(token,key)),key}function checkBinding$1(view,def,bindingIdx,value){var oldValue=view.oldValues[def.bindingIndex+bindingIdx];return!!(view.state&ViewState.FirstCheck)||!devModeEqual(oldValue,value)}function checkBindingNoChanges(view,def,bindingIdx,value){var oldValue=view.oldValues[def.bindingIndex+bindingIdx];if(view.state&ViewState.FirstCheck||!devModeEqual(oldValue,value))throw expressionChangedAfterItHasBeenCheckedError$1(Services.createDebugContext(view,def.index),oldValue,value,0!==(view.state&ViewState.FirstCheck))}function checkAndUpdateBinding(view,def,bindingIdx,value){var oldValues=view.oldValues;if(view.state&ViewState.FirstCheck||!looseIdentical(oldValues[def.bindingIndex+bindingIdx],value)){if(oldValues[def.bindingIndex+bindingIdx]=value,def.flags&NodeFlags.HasComponent){var compView=asProviderData(view,def.index).componentView;compView.def.flags&ViewFlags.OnPush&&(compView.state|=ViewState.ChecksEnabled)}return!0}return!1}function dispatchEvent(view,nodeIndex,eventName,event){for(var currView=view;currView;)currView.def.flags&ViewFlags.OnPush&&(currView.state|=ViewState.ChecksEnabled),currView=currView.parent;return Services.handleEvent(view,nodeIndex,eventName,event)}function unwrapValue(value){return value instanceof WrappedValue&&(value=value.wrapped),value}function declaredViewContainer(view){if(view.parent){var parentView=view.parent;return asElementData(parentView,view.parentIndex)}}function viewParentDiIndex(view){if(view.parent&&view.context!==view.component){var parentNodeDef=view.parent.def.nodes[view.parentIndex];return parentNodeDef.parent}return view.parentIndex}function renderNode(view,def){switch(def.type){case NodeType.Element:return asElementData(view,def.index).renderElement;case NodeType.Text:return asTextData(view,def.index).renderText}}function nodeValue(view,index){var def=view.def.nodes[index];switch(def.type){case NodeType.Element:return asElementData(view,def.index).renderElement;case NodeType.Text:return asTextData(view,def.index).renderText;case NodeType.Provider:return asProviderData(view,def.index).instance}}function queryIdIsReference(queryId){return queryId.startsWith("#")}function elementEventFullName(target,name){return target?target+":"+name:name}function isComponentView(view){return view.component===view.context&&!!view.parent}function resolveViewDefinition(factory){var value=VIEW_DEFINITION_CACHE.get(factory);return value||(value=factory(),VIEW_DEFINITION_CACHE.set(factory,value)),value}function sliceErrorStack(start,end){var err;try{throw new Error}catch(e){err=e}var stack=err.stack||"",lines=stack.split("\n");return lines[0].startsWith("Error")&&(start++,end++),lines.slice(start,end).join("\n")}function rootRenderNodes(view){var renderNodes=[];return visitRootRenderNodes(view,RenderNodeAction.Collect,void 0,void 0,renderNodes),renderNodes}function visitRootRenderNodes(view,action,parentNode,nextSibling,target){for(var len=view.def.nodes.length,i=0;i<len;i++){var nodeDef=view.def.nodes[i];visitRenderNode(view,nodeDef,action,parentNode,nextSibling,target),i+=nodeDef.childCount}}function visitProjectedRenderNodes(view,ngContentIndex,action,parentNode,nextSibling,target){for(var compView=view;compView&&!isComponentView(compView);)compView=compView.parent;for(var hostView=compView.parent,hostElDef=hostView.def.nodes[compView.parentIndex],startIndex=hostElDef.index+1,endIndex=hostElDef.index+hostElDef.childCount,i=startIndex;i<=endIndex;i++){var nodeDef=hostView.def.nodes[i];nodeDef.ngContentIndex===ngContentIndex&&visitRenderNode(hostView,nodeDef,action,parentNode,nextSibling,target),i+=nodeDef.childCount}if(!hostView.parent){var projectedNodes=view.root.projectableNodes[ngContentIndex];if(projectedNodes)for(var i=0;i<projectedNodes.length;i++)execRenderNodeAction(view,projectedNodes[i],action,parentNode,nextSibling,target)}}function visitRenderNode(view,nodeDef,action,parentNode,nextSibling,target){if(nodeDef.type===NodeType.NgContent)visitProjectedRenderNodes(view,nodeDef.ngContent.index,action,parentNode,nextSibling,target);else{var rn=renderNode(view,nodeDef);if(execRenderNodeAction(view,rn,action,parentNode,nextSibling,target),nodeDef.flags&NodeFlags.HasEmbeddedViews){var embeddedViews=asElementData(view,nodeDef.index).embeddedViews;if(embeddedViews)for(var k=0;k<embeddedViews.length;k++)visitRootRenderNodes(embeddedViews[k],action,parentNode,nextSibling,target)}}}function execRenderNodeAction(view,renderNode,action,parentNode,nextSibling,target){var renderer=view.root.renderer;switch(action){case RenderNodeAction.AppendChild:renderer.appendChild(parentNode,renderNode);break;case RenderNodeAction.InsertBefore:renderer.insertBefore(parentNode,renderNode,nextSibling);break;case RenderNodeAction.RemoveChild:renderer.removeChild(parentNode,renderNode);break;case RenderNodeAction.Collect:target.push(renderNode)}}function anchorDef(flags,matchedQueries,ngContentIndex,childCount,templateFactory){var matchedQueryDefs={};matchedQueries&&matchedQueries.forEach(function(_a){var queryId=_a[0],valueType=_a[1];matchedQueryDefs[queryId]=valueType});var source=isDevMode()?sliceErrorStack(2,3):"",template=templateFactory?resolveViewDefinition(templateFactory):null;return{type:NodeType.Element,index:void 0,reverseChildIndex:void 0,parent:void 0,childFlags:void 0,childMatchedQueries:void 0,bindingIndex:void 0,disposableIndex:void 0,flags:flags,matchedQueries:matchedQueryDefs,ngContentIndex:ngContentIndex,childCount:childCount,bindings:[],disposableCount:0,element:{name:void 0,attrs:void 0,outputs:[],template:template,providerIndices:void 0,source:source},provider:void 0,text:void 0,pureExpression:void 0,query:void 0,ngContent:void 0}}function elementDef(flags,matchedQueries,ngContentIndex,childCount,name,fixedAttrs,bindings,outputs){void 0===fixedAttrs&&(fixedAttrs={});var source=isDevMode()?sliceErrorStack(2,3):"",matchedQueryDefs={};matchedQueries&&matchedQueries.forEach(function(_a){var queryId=_a[0],valueType=_a[1];matchedQueryDefs[queryId]=valueType}),bindings=bindings||[];for(var bindingDefs=new Array(bindings.length),i=0;i<bindings.length;i++){var entry=bindings[i],bindingType=entry[0],name_1=entry[1],securityContext=void 0,suffix=void 0;switch(bindingType){case BindingType.ElementStyle:suffix=entry[2];break;case BindingType.ElementAttribute:case BindingType.ElementProperty:securityContext=entry[2]}bindingDefs[i]={type:bindingType,name:name_1,nonMinifiedName:name_1,securityContext:securityContext,suffix:suffix}}outputs=outputs||[];for(var outputDefs=new Array(outputs.length),i=0;i<outputs.length;i++){var output=outputs[i],target=void 0,eventName=void 0;Array.isArray(output)?(target=output[0],eventName=output[1]):eventName=output,outputDefs[i]={eventName:eventName,target:target}}return{type:NodeType.Element,index:void 0,reverseChildIndex:void 0,parent:void 0,childFlags:void 0,childMatchedQueries:void 0,bindingIndex:void 0,disposableIndex:void 0,flags:flags,matchedQueries:matchedQueryDefs,ngContentIndex:ngContentIndex,childCount:childCount,bindings:bindingDefs,disposableCount:outputDefs.length,element:{name:name,attrs:fixedAttrs,outputs:outputDefs,template:void 0,providerIndices:void 0,source:source},provider:void 0,text:void 0,pureExpression:void 0,query:void 0,ngContent:void 0}}function createElement(view,renderHost,def){var el,elDef=def.element,rootSelectorOrNode=view.root.selectorOrNode,renderer=view.root.renderer;if(view.parent||!rootSelectorOrNode){var parentNode=null!=def.parent?asElementData(view,def.parent).renderElement:renderHost;el=elDef.name?renderer.createElement(elDef.name):renderer.createComment(""),parentNode&&renderer.appendChild(parentNode,el)}else el=renderer.selectRootElement(rootSelectorOrNode);if(elDef.attrs)for(var attrName in elDef.attrs)renderer.setAttribute(el,attrName,elDef.attrs[attrName]);if(elDef.outputs.length)for(var i=0;i<elDef.outputs.length;i++){var output=elDef.outputs[i],handleEventClosure=renderEventHandlerClosure(view,def.index,elementEventFullName(output.target,output.eventName)),disposable=renderer.listen(output.target||el,output.eventName,handleEventClosure);view.disposables[def.disposableIndex+i]=disposable}return{renderElement:el,embeddedViews:def.flags&NodeFlags.HasEmbeddedViews?[]:void 0,projectedViews:void 0}}function renderEventHandlerClosure(view,index,eventName){return function(event){return dispatchEvent(view,index,eventName,event)}}function checkAndUpdateElementInline(view,def,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9){switch(def.bindings.length){case 10:checkAndUpdateElementValue(view,def,9,v9);case 9:checkAndUpdateElementValue(view,def,8,v8);case 8:checkAndUpdateElementValue(view,def,7,v7);case 7:checkAndUpdateElementValue(view,def,6,v6);case 6:checkAndUpdateElementValue(view,def,5,v5);case 5:checkAndUpdateElementValue(view,def,4,v4);case 4:checkAndUpdateElementValue(view,def,3,v3);case 3:checkAndUpdateElementValue(view,def,2,v2);case 2:checkAndUpdateElementValue(view,def,1,v1);case 1:checkAndUpdateElementValue(view,def,0,v0)}}function checkAndUpdateElementDynamic(view,def,values){for(var i=0;i<values.length;i++)checkAndUpdateElementValue(view,def,i,values[i])}function checkAndUpdateElementValue(view,def,bindingIdx,value){if(checkAndUpdateBinding(view,def,bindingIdx,value)){value=unwrapValue(value);var binding=def.bindings[bindingIdx],name=binding.name,renderNode=asElementData(view,def.index).renderElement;switch(binding.type){case BindingType.ElementAttribute:setElementAttribute(view,binding,renderNode,name,value);break;case BindingType.ElementClass:setElementClass(view,renderNode,name,value);break;case BindingType.ElementStyle:setElementStyle(view,binding,renderNode,name,value);break;case BindingType.ElementProperty:setElementProperty(view,binding,renderNode,name,value)}}}function setElementAttribute(view,binding,renderNode,name,value){var securityContext=binding.securityContext,renderValue=securityContext?view.root.sanitizer.sanitize(securityContext,value):value;renderValue=null!=renderValue?renderValue.toString():null;var renderer=view.root.renderer;null!=value?renderer.setAttribute(renderNode,name,renderValue):renderer.removeAttribute(renderNode,name)}function setElementClass(view,renderNode,name,value){var renderer=view.root.renderer;value?renderer.addClass(renderNode,name):renderer.removeClass(renderNode,name)}function setElementStyle(view,binding,renderNode,name,value){var renderValue=view.root.sanitizer.sanitize(SecurityContext.STYLE,value);if(null!=renderValue){renderValue=renderValue.toString();var unit=binding.suffix;null!=unit&&(renderValue+=unit)}else renderValue=null;var renderer=view.root.renderer;null!=renderValue?renderer.setStyle(renderNode,name,renderValue):renderer.removeStyle(renderNode,name)}function setElementProperty(view,binding,renderNode,name,value){var securityContext=binding.securityContext,renderValue=securityContext?view.root.sanitizer.sanitize(securityContext,value):value;view.root.renderer.setProperty(renderNode,name,renderValue)}function ngContentDef(ngContentIndex,index){return{type:NodeType.NgContent,index:void 0,reverseChildIndex:void 0,parent:void 0,childFlags:void 0,childMatchedQueries:void 0,bindingIndex:void 0,disposableIndex:void 0,flags:0,matchedQueries:{},ngContentIndex:ngContentIndex,childCount:0,bindings:[],disposableCount:0,element:void 0,provider:void 0,text:void 0,pureExpression:void 0,query:void 0,ngContent:{index:index}}}function appendNgContent(view,renderHost,def){if(null==def.ngContentIndex){var parentEl=null!=def.parent?asElementData(view,def.parent).renderElement:renderHost;if(parentEl){var ngContentIndex=def.ngContent.index;visitProjectedRenderNodes(view,ngContentIndex,RenderNodeAction.AppendChild,parentEl,void 0,void 0)}}}function createComponentFactory(selector,componentType,viewDefFactory){return new ComponentFactory_(selector,componentType,viewDefFactory)}function createViewContainerRef(view,elIndex){return new ViewContainerRef_$1(view,elIndex)}function createChangeDetectorRef(view){return new ViewRef_$1(view)}function createTemplateRef(view,def){return new TemplateRef_$1(view,def)}function createInjector(view,elIndex){return new Injector_(view,elIndex)}function directiveDef(flags,matchedQueries,childCount,ctor,deps,props,outputs,component){return _providerDef(flags,matchedQueries,childCount,ProviderType.Class,ctor,ctor,deps,props,outputs,component)}function providerDef(flags,matchedQueries,type,token,value,deps){return _providerDef(flags,matchedQueries,0,type,token,value,deps)}function _providerDef(flags,matchedQueries,childCount,type,token,value,deps,props,outputs,component){var matchedQueryDefs={};matchedQueries&&matchedQueries.forEach(function(_a){var queryId=_a[0],valueType=_a[1];matchedQueryDefs[queryId]=valueType});var bindings=[];if(props)for(var prop in props){var _a=props[prop],bindingIndex=_a[0],nonMinifiedName=_a[1];bindings[bindingIndex]={type:BindingType.ProviderProperty,name:prop,nonMinifiedName:nonMinifiedName,securityContext:void 0,suffix:void 0}}var outputDefs=[];if(outputs)for(var propName in outputs)outputDefs.push({propName:propName,eventName:outputs[propName]});var depDefs=deps.map(function(value){var token,flags;return Array.isArray(value)?(flags=value[0],token=value[1]):(flags=DepFlags.None,token=value),{flags:flags,token:token,tokenKey:tokenKey(token)}});return component&&(flags|=NodeFlags.HasComponent),{type:NodeType.Provider,index:void 0,reverseChildIndex:void 0,parent:void 0,childFlags:void 0,childMatchedQueries:void 0,bindingIndex:void 0,disposableIndex:void 0,flags:flags,matchedQueries:matchedQueryDefs,ngContentIndex:void 0,childCount:childCount,bindings:bindings,disposableCount:outputDefs.length,element:void 0,provider:{type:type,token:token,tokenKey:tokenKey(token),value:value,deps:depDefs,outputs:outputDefs,component:component},text:void 0,pureExpression:void 0,query:void 0,ngContent:void 0}}function createProviderInstance(view,def){def.provider;return def.flags&NodeFlags.LazyProvider?NOT_CREATED:createInstance(view,def)}function eventHandlerClosure(view,index,eventName){return function(event){return dispatchEvent(view,index,eventName,event)}}function checkAndUpdateProviderInline(view,def,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9){var changes,provider=asProviderData(view,def.index).instance;switch(def.bindings.length){case 10:changes=checkAndUpdateProp(view,provider,def,9,v9,changes);case 9:changes=checkAndUpdateProp(view,provider,def,8,v8,changes);case 8:changes=checkAndUpdateProp(view,provider,def,7,v7,changes);case 7:changes=checkAndUpdateProp(view,provider,def,6,v6,changes);case 6:changes=checkAndUpdateProp(view,provider,def,5,v5,changes);case 5:changes=checkAndUpdateProp(view,provider,def,4,v4,changes);case 4:changes=checkAndUpdateProp(view,provider,def,3,v3,changes);case 3:changes=checkAndUpdateProp(view,provider,def,2,v2,changes);case 2:changes=checkAndUpdateProp(view,provider,def,1,v1,changes);case 1:changes=checkAndUpdateProp(view,provider,def,0,v0,changes)}changes&&provider.ngOnChanges(changes),view.state&ViewState.FirstCheck&&def.flags&NodeFlags.OnInit&&provider.ngOnInit(),def.flags&NodeFlags.DoCheck&&provider.ngDoCheck()}function checkAndUpdateProviderDynamic(view,def,values){for(var changes,provider=asProviderData(view,def.index).instance,i=0;i<values.length;i++)changes=checkAndUpdateProp(view,provider,def,i,values[i],changes);changes&&provider.ngOnChanges(changes),view.state&ViewState.FirstCheck&&def.flags&NodeFlags.OnInit&&provider.ngOnInit(),def.flags&NodeFlags.DoCheck&&provider.ngDoCheck()}function createInstance(view,nodeDef){var injectable,providerDef=nodeDef.provider;switch(providerDef.type){case ProviderType.Class:injectable=createClass(view,nodeDef.index,nodeDef.parent,providerDef.value,providerDef.deps);break;case ProviderType.Factory:injectable=callFactory(view,nodeDef.index,nodeDef.parent,providerDef.value,providerDef.deps);break;case ProviderType.UseExisting:injectable=resolveDep(view,nodeDef.index,nodeDef.parent,providerDef.deps[0]);break;case ProviderType.Value:injectable=providerDef.value}if(providerDef.outputs.length)for(var i=0;i<providerDef.outputs.length;i++){var output=providerDef.outputs[i],subscription=injectable[output.propName].subscribe(eventHandlerClosure(view,nodeDef.parent,output.eventName));view.disposables[nodeDef.disposableIndex+i]=subscription.unsubscribe.bind(subscription)}return injectable}function createClass(view,requestorNodeIndex,elIndex,ctor,deps){var injectable,len=deps.length;switch(len){case 0:injectable=new ctor;break;case 1:injectable=new ctor(resolveDep(view,requestorNodeIndex,elIndex,deps[0]));break;case 2:injectable=new ctor(resolveDep(view,requestorNodeIndex,elIndex,deps[0]),resolveDep(view,requestorNodeIndex,elIndex,deps[1]));break;case 3:injectable=new ctor(resolveDep(view,requestorNodeIndex,elIndex,deps[0]),resolveDep(view,requestorNodeIndex,elIndex,deps[1]),resolveDep(view,requestorNodeIndex,elIndex,deps[2]));break;default:for(var depValues=new Array(len),i=0;i<len;i++)depValues[i]=resolveDep(view,requestorNodeIndex,elIndex,deps[i]);injectable=new(ctor.bind.apply(ctor,[void 0].concat(depValues)))}return injectable}function callFactory(view,requestorNodeIndex,elIndex,factory,deps){var injectable,len=deps.length;switch(len){case 0:injectable=factory();break;case 1:injectable=factory(resolveDep(view,requestorNodeIndex,elIndex,deps[0]));break;case 2:injectable=factory(resolveDep(view,requestorNodeIndex,elIndex,deps[0]),resolveDep(view,requestorNodeIndex,elIndex,deps[1]));break;case 3:injectable=factory(resolveDep(view,requestorNodeIndex,elIndex,deps[0]),resolveDep(view,requestorNodeIndex,elIndex,deps[1]),resolveDep(view,requestorNodeIndex,elIndex,deps[2]));break;default:for(var depValues=Array(len),i=0;i<len;i++)depValues[i]=resolveDep(view,requestorNodeIndex,elIndex,deps[i]);injectable=factory.apply(void 0,depValues)}return injectable}function resolveDep(view,requestNodeIndex,elIndex,depDef,notFoundValue){if(void 0===notFoundValue&&(notFoundValue=Injector.THROW_IF_NOT_FOUND),depDef.flags&DepFlags.Value)return depDef.token;var startView=view;depDef.flags&DepFlags.Optional&&(notFoundValue=null);var tokenKey=depDef.tokenKey;if(depDef.flags&DepFlags.SkipSelf)for(requestNodeIndex=null,elIndex=view.def.nodes[elIndex].parent;null==elIndex&&view;)elIndex=viewParentDiIndex(view),view=view.parent;for(;view;){var elDef=view.def.nodes[elIndex];switch(tokenKey){case RendererV1TokenKey:for(var compView=view;compView&&!isComponentView(compView);)compView=compView.parent;var rootRenderer=view.root.injector.get(RootRenderer);return rootRenderer.renderComponent(new RenderComponentType(view.def.component.id,"",0,view.def.component.encapsulation,[],{}));case ElementRefTokenKey:return new ElementRef(asElementData(view,elIndex).renderElement);case ViewContainerRefTokenKey:return createViewContainerRef(view,elIndex);case TemplateRefTokenKey:return createTemplateRef(view,elDef);case ChangeDetectorRefTokenKey:var cdView=view;if(null!=requestNodeIndex){var requestorNodeDef=view.def.nodes[requestNodeIndex];requestorNodeDef.flags&NodeFlags.HasComponent&&(cdView=asProviderData(view,requestNodeIndex).componentView)}return createChangeDetectorRef(cdView);case InjectorRefTokenKey:return createInjector(view,elIndex);default:var providerIndex=elDef.element.providerIndices[tokenKey];if(null!=providerIndex){var providerData=asProviderData(view,providerIndex);return providerData.instance===NOT_CREATED&&(providerData.instance=createInstance(view,view.def.nodes[providerIndex])),providerData.instance}}requestNodeIndex=null,elIndex=viewParentDiIndex(view),view=view.parent}return startView.root.injector.get(depDef.token,notFoundValue)}function checkAndUpdateProp(view,provider,def,bindingIdx,value,changes){var change,changed;if(def.flags&NodeFlags.OnChanges){var oldValue=view.oldValues[def.bindingIndex+bindingIdx];changed=checkAndUpdateBinding(view,def,bindingIdx,value),change=changed?new SimpleChange(oldValue,value,0!==(view.state&ViewState.FirstCheck)):null}else changed=checkAndUpdateBinding(view,def,bindingIdx,value);if(changed){value=unwrapValue(value);var binding=def.bindings[bindingIdx],propName=binding.name;provider[propName]=value,change&&(changes=changes||{},changes[binding.nonMinifiedName]=change)}return changes}function callLifecycleHooksChildrenFirst(view,lifecycles){if(view.def.nodeFlags&lifecycles)for(var len=view.def.nodes.length,i=0;i<len;i++){var nodeDef=view.def.reverseChildNodes[i],nodeIndex=nodeDef.index;nodeDef.flags&lifecycles?(Services.setCurrentNode(view,nodeIndex),callProviderLifecycles(asProviderData(view,nodeIndex).instance,nodeDef.flags&lifecycles)):0===(nodeDef.childFlags&lifecycles)&&(i+=nodeDef.childCount)}}function callProviderLifecycles(provider,lifecycles){lifecycles&NodeFlags.AfterContentInit&&provider.ngAfterContentInit(),lifecycles&NodeFlags.AfterContentChecked&&provider.ngAfterContentChecked(),lifecycles&NodeFlags.AfterViewInit&&provider.ngAfterViewInit(),lifecycles&NodeFlags.AfterViewChecked&&provider.ngAfterViewChecked(),lifecycles&NodeFlags.OnDestroy&&provider.ngOnDestroy()}function purePipeDef(pipeToken,argCount){return _pureExpressionDef(PureExpressionType.Pipe,new Array(argCount),{token:pipeToken,tokenKey:tokenKey(pipeToken),flags:DepFlags.None})}function pureArrayDef(argCount){return _pureExpressionDef(PureExpressionType.Array,new Array(argCount),void 0)}function pureObjectDef(propertyNames){return _pureExpressionDef(PureExpressionType.Object,propertyNames,void 0)}function _pureExpressionDef(type,propertyNames,pipeDep){for(var bindings=new Array(propertyNames.length),i=0;i<propertyNames.length;i++){var prop=propertyNames[i];bindings[i]={type:BindingType.PureExpressionProperty,name:prop,nonMinifiedName:prop,securityContext:void 0,suffix:void 0}}return{type:NodeType.PureExpression,index:void 0,reverseChildIndex:void 0,parent:void 0,childFlags:void 0,childMatchedQueries:void 0,bindingIndex:void 0,disposableIndex:void 0,flags:0,matchedQueries:{},ngContentIndex:void 0,childCount:0,bindings:bindings,disposableCount:0,element:void 0,provider:void 0,text:void 0,pureExpression:{type:type,pipeDep:pipeDep},query:void 0,ngContent:void 0}}function createPureExpression(view,def){var pipe=def.pureExpression.pipeDep?Services.resolveDep(view,def.index,def.parent,def.pureExpression.pipeDep):void 0;return{value:void 0,pipe:pipe}}function checkAndUpdatePureExpressionInline(view,def,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9){var bindings=def.bindings,changed=!1;switch(bindings.length){case 10:checkAndUpdateBinding(view,def,9,v9)&&(changed=!0);case 9:checkAndUpdateBinding(view,def,8,v8)&&(changed=!0);case 8:checkAndUpdateBinding(view,def,7,v7)&&(changed=!0);case 7:checkAndUpdateBinding(view,def,6,v6)&&(changed=!0);case 6:checkAndUpdateBinding(view,def,5,v5)&&(changed=!0);case 5:checkAndUpdateBinding(view,def,4,v4)&&(changed=!0);case 4:checkAndUpdateBinding(view,def,3,v3)&&(changed=!0);case 3:checkAndUpdateBinding(view,def,2,v2)&&(changed=!0);case 2:checkAndUpdateBinding(view,def,1,v1)&&(changed=!0);case 1:checkAndUpdateBinding(view,def,0,v0)&&(changed=!0)}var data=asPureExpressionData(view,def.index);if(changed){v0=unwrapValue(v0),v1=unwrapValue(v1),v2=unwrapValue(v2),v3=unwrapValue(v3),v4=unwrapValue(v4),v5=unwrapValue(v5),v6=unwrapValue(v6),v7=unwrapValue(v7),v8=unwrapValue(v8),v9=unwrapValue(v9);var value=void 0;switch(def.pureExpression.type){case PureExpressionType.Array:switch(value=new Array(bindings.length),bindings.length){case 10:value[9]=v9;case 9:value[8]=v8;case 8:value[7]=v7;case 7:value[6]=v6;case 6:value[5]=v5;case 5:value[4]=v4;case 4:value[3]=v3;case 3:value[2]=v2;case 2:value[1]=v1;case 1:value[0]=v0}break;case PureExpressionType.Object:switch(value={},bindings.length){case 10:value[bindings[9].name]=v9;case 9:value[bindings[8].name]=v8;case 8:value[bindings[7].name]=v7;case 7:value[bindings[6].name]=v6;case 6:value[bindings[5].name]=v5;case 5:value[bindings[4].name]=v4;case 4:value[bindings[3].name]=v3;case 3:value[bindings[2].name]=v2;case 2:value[bindings[1].name]=v1;case 1:value[bindings[0].name]=v0}break;case PureExpressionType.Pipe:switch(bindings.length){case 10:value=data.pipe.transform(v0,v1,v2,v3,v4,v5,v6,v7,v8,v9);break;case 9:value=data.pipe.transform(v0,v1,v2,v3,v4,v5,v6,v7,v8);break;case 8:value=data.pipe.transform(v0,v1,v2,v3,v4,v5,v6,v7);break;case 7:value=data.pipe.transform(v0,v1,v2,v3,v4,v5,v6);break;case 6:value=data.pipe.transform(v0,v1,v2,v3,v4,v5);break;case 5:value=data.pipe.transform(v0,v1,v2,v3,v4);break;case 4:value=data.pipe.transform(v0,v1,v2,v3);break;case 3:value=data.pipe.transform(v0,v1,v2);break;case 2:value=data.pipe.transform(v0,v1);break;case 1:value=data.pipe.transform(v0)}}data.value=value}return data.value}function checkAndUpdatePureExpressionDynamic(view,def,values){for(var bindings=def.bindings,changed=!1,i=0;i<values.length;i++)checkAndUpdateBinding(view,def,i,values[i])&&(changed=!0);var data=asPureExpressionData(view,def.index);if(changed){var value=void 0;switch(def.pureExpression.type){case PureExpressionType.Array:value=new Array(values.length);for(var i=0;i<values.length;i++)value[i]=unwrapValue(values[i]);break;case PureExpressionType.Object:value={};for(var i=0;i<values.length;i++)value[bindings[i].name]=unwrapValue(values[i]);break;case PureExpressionType.Pipe:for(var params=new Array(values.length),i=0;i<values.length;i++)params[i]=unwrapValue(values[i]);value=(_a=data.pipe).transform.apply(_a,params)}data.value=value}return data.value;var _a}function queryDef(flags,id,bindings){var bindingDefs=[];for(var propName in bindings){var bindingType=bindings[propName];bindingDefs.push({propName:propName,bindingType:bindingType})}return{type:NodeType.Query,index:void 0,reverseChildIndex:void 0,parent:void 0,childFlags:void 0,childMatchedQueries:void 0,bindingIndex:void 0,disposableIndex:void 0,flags:flags,ngContentIndex:void 0,matchedQueries:{},childCount:0,bindings:[],disposableCount:0,element:void 0,provider:void 0,text:void 0,pureExpression:void 0,query:{id:id,bindings:bindingDefs},ngContent:void 0}}function createQuery(){return new QueryList}function dirtyParentQuery(queryId,view){var nodeIndex=view.parentIndex;view=view.parent;for(var queryIdx;view;){var elementDef=view.def.nodes[nodeIndex];if(queryIdx=elementDef.element.providerIndices[queryId],null!=queryIdx)break;nodeIndex=view.parentIndex,view=view.parent}if(!view)throw new Error("Illegal State: Tried to dirty parent query "+queryId+" but the query could not be found!");asQueryList(view,queryIdx).setDirty()}function checkAndUpdateQuery(view,nodeDef){var queryList=asQueryList(view,nodeDef.index);if(queryList.dirty){var newValues,queryId=nodeDef.query.id,providerDef=view.def.nodes[nodeDef.parent],providerData=asProviderData(view,providerDef.index);if(nodeDef.flags&NodeFlags.HasContentQuery){var elementDef=view.def.nodes[providerDef.parent];newValues=calcQueryValues(view,elementDef.index,elementDef.index+elementDef.childCount,queryId,[])}else if(nodeDef.flags&NodeFlags.HasViewQuery){var compView=providerData.componentView;newValues=calcQueryValues(compView,0,compView.def.nodes.length-1,queryId,[])}queryList.reset(newValues);for(var boundValue,bindings=nodeDef.query.bindings,i=0;i<bindings.length;i++){var binding=bindings[i];switch(binding.bindingType){case QueryBindingType.First:boundValue=queryList.first;break;case QueryBindingType.All:boundValue=queryList}providerData.instance[binding.propName]=boundValue}}}function calcQueryValues(view,startIndex,endIndex,queryId,values){for(var i=(view.def.nodes.length,startIndex);i<=endIndex;i++){var nodeDef=view.def.nodes[i],value=getQueryValue(view,nodeDef,queryId);if(null!=value&&values.push(value),nodeDef.flags&NodeFlags.HasEmbeddedViews&&queryId in nodeDef.element.template.nodeMatchedQueries){for(var elementData=asElementData(view,i),embeddedViews=elementData.embeddedViews,k=0;k<embeddedViews.length;k++){var embeddedView=embeddedViews[k],dvc=declaredViewContainer(embeddedView);dvc&&dvc===elementData&&calcQueryValues(embeddedView,0,embeddedView.def.nodes.length-1,queryId,values)}var projectedViews=elementData.projectedViews;if(projectedViews)for(var k=0;k<projectedViews.length;k++){var projectedView=projectedViews[k];calcQueryValues(projectedView,0,projectedView.def.nodes.length-1,queryId,values)}}queryId in nodeDef.childMatchedQueries||(i+=nodeDef.childCount)}return values}function getQueryValue(view,nodeDef,queryId){var queryValueType=nodeDef.matchedQueries[queryId];if(null!=queryValueType){var value=void 0;switch(queryValueType){case QueryValueType.RenderElement:value=asElementData(view,nodeDef.index).renderElement;break;case QueryValueType.ElementRef:value=new ElementRef(asElementData(view,nodeDef.index).renderElement);break;case QueryValueType.TemplateRef:value=createTemplateRef(view,nodeDef);break;case QueryValueType.ViewContainerRef:value=createViewContainerRef(view,nodeDef.index);break;case QueryValueType.Provider:value=asProviderData(view,nodeDef.index).instance}return value}}function textDef(ngContentIndex,constants){for(var source=isDevMode()?sliceErrorStack(2,3):"",bindings=new Array(constants.length-1),i=1;i<constants.length;i++)bindings[i-1]={type:BindingType.Interpolation,name:void 0,nonMinifiedName:void 0,securityContext:void 0,suffix:constants[i]};return{type:NodeType.Text,index:void 0,reverseChildIndex:void 0,parent:void 0,childFlags:void 0,childMatchedQueries:void 0,bindingIndex:void 0,disposableIndex:void 0,flags:0,matchedQueries:{},ngContentIndex:ngContentIndex,childCount:0,bindings:bindings,disposableCount:0,element:void 0,provider:void 0,text:{prefix:constants[0],source:source},pureExpression:void 0,query:void 0,ngContent:void 0}}function createText(view,renderHost,def){var renderNode,parentNode=null!=def.parent?asElementData(view,def.parent).renderElement:renderHost,renderer=view.root.renderer;return renderNode=renderer.createText(def.text.prefix),parentNode&&renderer.appendChild(parentNode,renderNode),{renderText:renderNode}}function checkAndUpdateTextInline(view,def,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9){var bindings=def.bindings,changed=!1;switch(bindings.length){case 10:checkAndUpdateBinding(view,def,9,v9)&&(changed=!0);case 9:checkAndUpdateBinding(view,def,8,v8)&&(changed=!0);case 8:checkAndUpdateBinding(view,def,7,v7)&&(changed=!0);case 7:checkAndUpdateBinding(view,def,6,v6)&&(changed=!0);case 6:checkAndUpdateBinding(view,def,5,v5)&&(changed=!0);case 5:checkAndUpdateBinding(view,def,4,v4)&&(changed=!0);case 4:checkAndUpdateBinding(view,def,3,v3)&&(changed=!0);case 3:checkAndUpdateBinding(view,def,2,v2)&&(changed=!0);case 2:checkAndUpdateBinding(view,def,1,v1)&&(changed=!0);case 1:checkAndUpdateBinding(view,def,0,v0)&&(changed=!0)}if(changed){var value="";switch(bindings.length){case 10:value=_addInterpolationPart(v9,bindings[9]);case 9:value=_addInterpolationPart(v8,bindings[8])+value;case 8:value=_addInterpolationPart(v7,bindings[7])+value;case 7:value=_addInterpolationPart(v6,bindings[6])+value;case 6:value=_addInterpolationPart(v5,bindings[5])+value;case 5:value=_addInterpolationPart(v4,bindings[4])+value;case 4:value=_addInterpolationPart(v3,bindings[3])+value;case 3:value=_addInterpolationPart(v2,bindings[2])+value;case 2:value=_addInterpolationPart(v1,bindings[1])+value;case 1:value=_addInterpolationPart(v0,bindings[0])+value}value=def.text.prefix+value;var renderNode=asTextData(view,def.index).renderText;view.root.renderer.setText(renderNode,value)}}function checkAndUpdateTextDynamic(view,def,values){for(var bindings=def.bindings,changed=!1,i=0;i<values.length;i++)checkAndUpdateBinding(view,def,i,values[i])&&(changed=!0);if(changed){for(var value="",i=0;i<values.length;i++)value+=_addInterpolationPart(values[i],bindings[i]);
value=def.text.prefix+value;var renderNode=asTextData(view,def.index).renderText;view.root.renderer.setText(renderNode,value)}}function _addInterpolationPart(value,binding){value=unwrapValue(value);var valueStr=null!=value?value.toString():"";return valueStr+binding.suffix}function viewDef(flags,nodesWithoutIndices,update,handleEvent,compId,encapsulation,styles){if(0===nodesWithoutIndices.length)throw new Error("Illegal State: Views without nodes are not allowed!");for(var nodes=new Array(nodesWithoutIndices.length),reverseChildNodes=new Array(nodesWithoutIndices.length),viewBindingCount=0,viewDisposableCount=0,viewNodeFlags=0,viewMatchedQueries={},currentParent=null,lastRootNode=null,i=0;i<nodesWithoutIndices.length;i++){for(;currentParent&&i>currentParent.index+currentParent.childCount;){var newParent=nodes[currentParent.parent];newParent&&(newParent.childFlags|=currentParent.childFlags,copyQueryMatchesInto(currentParent.childMatchedQueries,newParent.childMatchedQueries)),currentParent=newParent}var nodeWithoutIndices=nodesWithoutIndices[i],reverseChildIndex=calculateReverseChildIndex(currentParent,i,nodeWithoutIndices.childCount,nodesWithoutIndices.length),node=cloneAndModifyNode(nodeWithoutIndices,{index:i,parent:currentParent?currentParent.index:void 0,bindingIndex:viewBindingCount,disposableIndex:viewDisposableCount,reverseChildIndex:reverseChildIndex});if(node.element&&(node.element=cloneAndModifyElement(node.element,{providerIndices:Object.create(currentParent?currentParent.element.providerIndices:null)})),nodes[i]=node,reverseChildNodes[reverseChildIndex]=node,validateNode(currentParent,node,nodesWithoutIndices.length),viewNodeFlags|=node.flags,copyQueryMatchesInto(node.matchedQueries,viewMatchedQueries),viewBindingCount+=node.bindings.length,viewDisposableCount+=node.disposableCount,currentParent&&(currentParent.childFlags|=node.flags,copyQueryMatchesInto(node.matchedQueries,currentParent.childMatchedQueries),node.element&&node.element.template&&copyQueryMatchesInto(node.element.template.nodeMatchedQueries,currentParent.childMatchedQueries)),currentParent||(lastRootNode=node),node.provider&&(currentParent.element.providerIndices[node.provider.tokenKey]=i),node.query){var elementDef=nodes[currentParent.parent];elementDef.element.providerIndices[node.query.id]=i}node.childCount&&(currentParent=node)}for(;currentParent;){var newParent=nodes[currentParent.parent];newParent&&(newParent.childFlags|=currentParent.childFlags,copyQueryMatchesInto(currentParent.childMatchedQueries,newParent.childMatchedQueries)),currentParent=newParent}var componentDef=compId?{id:compId,encapsulation:encapsulation,styles:styles}:void 0;return{nodeFlags:viewNodeFlags,nodeMatchedQueries:viewMatchedQueries,flags:flags,nodes:nodes,reverseChildNodes:reverseChildNodes,update:update||NOOP,handleEvent:handleEvent||NOOP,component:componentDef,bindingCount:viewBindingCount,disposableCount:viewDisposableCount,lastRootNode:lastRootNode}}function copyQueryMatchesInto(source,target){for(var prop in source)queryIdIsReference(prop)||(target[prop]=!0)}function calculateReverseChildIndex(currentParent,i,childCount,nodeCount){var lastChildOffsetRelativeToParentInDfsOrder,parentEndIndexInReverseChildOrder;if(currentParent){var lastChildIndex=i+childCount;lastChildOffsetRelativeToParentInDfsOrder=lastChildIndex-currentParent.index-1,parentEndIndexInReverseChildOrder=currentParent.reverseChildIndex+currentParent.childCount}else lastChildOffsetRelativeToParentInDfsOrder=i+childCount,parentEndIndexInReverseChildOrder=nodeCount-1;return parentEndIndexInReverseChildOrder-lastChildOffsetRelativeToParentInDfsOrder}function validateNode(parent,node,nodeCount){var template=node.element&&node.element.template;if(template&&template.lastRootNode&&template.lastRootNode.flags&NodeFlags.HasEmbeddedViews)throw new Error("Illegal State: Last root node of a template can't have embedded views, at index "+node.index+"!");if(node.provider){var parentType=parent?parent.type:null;if(parentType!==NodeType.Element)throw new Error("Illegal State: Provider nodes need to be children of elements or anchors, at index "+node.index+"!")}if(node.query){var parentType=parent?parent.type:null;if(parentType!==NodeType.Provider)throw new Error("Illegal State: Query nodes need to be children of providers, at index "+node.index+"!")}if(node.childCount){var parentEnd=parent?parent.index+parent.childCount:nodeCount-1;if(node.index<=parentEnd&&node.index+node.childCount>parentEnd)throw new Error("Illegal State: childCount of node leads outside of parent, at index "+node.index+"!")}}function cloneAndModifyNode(nodeDef,values){return{type:nodeDef.type,index:values.index,reverseChildIndex:values.reverseChildIndex,parent:values.parent,childFlags:0,childMatchedQueries:{},bindingIndex:values.bindingIndex,disposableIndex:values.disposableIndex,flags:nodeDef.flags,matchedQueries:nodeDef.matchedQueries,ngContentIndex:nodeDef.ngContentIndex,childCount:nodeDef.childCount,bindings:nodeDef.bindings,disposableCount:nodeDef.disposableCount,element:nodeDef.element,provider:nodeDef.provider,text:nodeDef.text,pureExpression:nodeDef.pureExpression,query:nodeDef.query,ngContent:nodeDef.ngContent}}function cloneAndModifyElement(elementDef,values){return{name:elementDef.name,attrs:elementDef.attrs,outputs:elementDef.outputs,template:elementDef.template,providerIndices:values.providerIndices,source:elementDef.source}}function createEmbeddedView(parent,anchorDef,context){var view=createView(parent.root,parent,anchorDef.index,anchorDef.element.template);return initView(view,parent.component,context),createViewNodes(view),view}function createRootView(root,def,context){var view=createView(root,null,null,def);return initView(view,context,context),createViewNodes(view),view}function createView(root,parent,parentIndex,def){var nodes=new Array(def.nodes.length),disposables=def.disposableCount?new Array(def.disposableCount):void 0,view={def:def,parent:parent,parentIndex:parentIndex,context:void 0,component:void 0,nodes:nodes,state:ViewState.FirstCheck|ViewState.ChecksEnabled,root:root,oldValues:new Array(def.bindingCount),disposables:disposables};return view}function initView(view,component,context){view.component=component,view.context=context}function createViewNodes(view){var renderHost;isComponentView(view)&&(renderHost=asElementData(view.parent,view.parentIndex).renderElement);for(var def=view.def,nodes=view.nodes,i=0;i<def.nodes.length;i++){var nodeDef=def.nodes[i];switch(Services.setCurrentNode(view,i),nodeDef.type){case NodeType.Element:nodes[i]=createElement(view,renderHost,nodeDef);break;case NodeType.Text:nodes[i]=createText(view,renderHost,nodeDef);break;case NodeType.Provider:if(nodeDef.provider.component){var componentView=createView(view.root,view,nodeDef.parent,resolveViewDefinition(nodeDef.provider.component)),providerData={componentView:componentView,instance:void 0};nodes[i]=providerData;var instance=providerData.instance=createProviderInstance(view,nodeDef);initView(componentView,instance,instance)}else{var instance=createProviderInstance(view,nodeDef),providerData={componentView:void 0,instance:instance};nodes[i]=providerData}break;case NodeType.PureExpression:nodes[i]=createPureExpression(view,nodeDef);break;case NodeType.Query:nodes[i]=createQuery();break;case NodeType.NgContent:appendNgContent(view,renderHost,nodeDef),nodes[i]=void 0}}execComponentViewsAction(view,ViewAction.CreateViewNodes)}function checkNoChangesView(view){Services.updateView(checkNoChangesNode,view),execEmbeddedViewsAction(view,ViewAction.CheckNoChanges),execQueriesAction(view,NodeFlags.HasContentQuery,QueryAction.CheckNoChanges),execComponentViewsAction(view,ViewAction.CheckNoChanges),execQueriesAction(view,NodeFlags.HasViewQuery,QueryAction.CheckNoChanges)}function checkAndUpdateView(view){Services.updateView(checkAndUpdateNode,view),execEmbeddedViewsAction(view,ViewAction.CheckAndUpdate),execQueriesAction(view,NodeFlags.HasContentQuery,QueryAction.CheckAndUpdate),callLifecycleHooksChildrenFirst(view,NodeFlags.AfterContentChecked|(view.state&ViewState.FirstCheck?NodeFlags.AfterContentInit:0)),execComponentViewsAction(view,ViewAction.CheckAndUpdate),execQueriesAction(view,NodeFlags.HasViewQuery,QueryAction.CheckAndUpdate),callLifecycleHooksChildrenFirst(view,NodeFlags.AfterViewChecked|(view.state&ViewState.FirstCheck?NodeFlags.AfterViewInit:0)),view.def.flags&ViewFlags.OnPush&&(view.state&=~ViewState.ChecksEnabled),view.state&=~ViewState.FirstCheck}function checkAndUpdateNode(view,nodeIndex,argStyle,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9){return argStyle===ArgumentType.Inline?checkAndUpdateNodeInline(view,nodeIndex,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9):checkAndUpdateNodeDynamic(view,nodeIndex,v0)}function checkAndUpdateNodeInline(view,nodeIndex,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9){var nodeDef=view.def.nodes[nodeIndex];switch(nodeDef.type){case NodeType.Element:return checkAndUpdateElementInline(view,nodeDef,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9);case NodeType.Text:return checkAndUpdateTextInline(view,nodeDef,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9);case NodeType.Provider:return checkAndUpdateProviderInline(view,nodeDef,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9);case NodeType.PureExpression:return checkAndUpdatePureExpressionInline(view,nodeDef,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9)}}function checkAndUpdateNodeDynamic(view,nodeIndex,values){var nodeDef=view.def.nodes[nodeIndex];switch(nodeDef.type){case NodeType.Element:return checkAndUpdateElementDynamic(view,nodeDef,values);case NodeType.Text:return checkAndUpdateTextDynamic(view,nodeDef,values);case NodeType.Provider:return checkAndUpdateProviderDynamic(view,nodeDef,values);case NodeType.PureExpression:return checkAndUpdatePureExpressionDynamic(view,nodeDef,values)}}function checkNoChangesNode(view,nodeIndex,argStyle,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9){return argStyle===ArgumentType.Inline?checkNoChangesNodeInline(view,nodeIndex,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9):checkNoChangesNodeDynamic(view,nodeIndex,v0)}function checkNoChangesNodeInline(view,nodeIndex,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9){var nodeDef=view.def.nodes[nodeIndex];switch(nodeDef.bindings.length){case 10:checkBindingNoChanges(view,nodeDef,9,v9);case 9:checkBindingNoChanges(view,nodeDef,8,v8);case 8:checkBindingNoChanges(view,nodeDef,7,v7);case 7:checkBindingNoChanges(view,nodeDef,6,v6);case 6:checkBindingNoChanges(view,nodeDef,5,v5);case 5:checkBindingNoChanges(view,nodeDef,4,v4);case 4:checkBindingNoChanges(view,nodeDef,3,v3);case 3:checkBindingNoChanges(view,nodeDef,2,v2);case 2:checkBindingNoChanges(view,nodeDef,1,v1);case 1:checkBindingNoChanges(view,nodeDef,0,v0)}return nodeDef.type===NodeType.PureExpression?asPureExpressionData(view,nodeIndex).value:void 0}function checkNoChangesNodeDynamic(view,nodeIndex,values){for(var nodeDef=view.def.nodes[nodeIndex],i=0;i<values.length;i++)checkBindingNoChanges(view,nodeDef,i,values[i]);return nodeDef.type===NodeType.PureExpression?asPureExpressionData(view,nodeIndex).value:void 0}function checkNoChangesQuery(view,nodeDef){var queryList=asQueryList(view,nodeDef.index);if(queryList.dirty)throw expressionChangedAfterItHasBeenCheckedError$1(Services.createDebugContext(view,nodeDef.index),"Query "+nodeDef.query.id+" not dirty","Query "+nodeDef.query.id+" dirty",0!==(view.state&ViewState.FirstCheck))}function destroyView(view){if(callLifecycleHooksChildrenFirst(view,NodeFlags.OnDestroy),view.disposables)for(var i=0;i<view.disposables.length;i++)view.disposables[i]();execComponentViewsAction(view,ViewAction.Destroy),execEmbeddedViewsAction(view,ViewAction.Destroy),view.state|=ViewState.Destroyed}function execComponentViewsAction(view,action){var def=view.def;if(def.nodeFlags&NodeFlags.HasComponent)for(var i=0;i<def.nodes.length;i++){var nodeDef=def.nodes[i];if(nodeDef.flags&NodeFlags.HasComponent){var providerData=asProviderData(view,i);callViewAction(providerData.componentView,action)}else 0===(nodeDef.childFlags&NodeFlags.HasComponent)&&(i+=nodeDef.childCount)}}function execEmbeddedViewsAction(view,action){var def=view.def;if(def.nodeFlags&NodeFlags.HasEmbeddedViews)for(var i=0;i<def.nodes.length;i++){var nodeDef=def.nodes[i];if(nodeDef.flags&NodeFlags.HasEmbeddedViews){var embeddedViews=asElementData(view,i).embeddedViews;if(embeddedViews)for(var k=0;k<embeddedViews.length;k++)callViewAction(embeddedViews[k],action)}else 0===(nodeDef.childFlags&NodeFlags.HasEmbeddedViews)&&(i+=nodeDef.childCount)}}function callViewAction(view,action){var viewState=view.state;switch(action){case ViewAction.CheckNoChanges:viewState&ViewState.ChecksEnabled&&0===(viewState&(ViewState.Errored|ViewState.Destroyed))&&checkNoChangesView(view);break;case ViewAction.CheckAndUpdate:viewState&ViewState.ChecksEnabled&&0===(viewState&(ViewState.Errored|ViewState.Destroyed))&&checkAndUpdateView(view);break;case ViewAction.Destroy:destroyView(view);break;case ViewAction.CreateViewNodes:createViewNodes(view)}}function execQueriesAction(view,queryFlags,action){if(view.def.nodeFlags&queryFlags)for(var nodeCount=view.def.nodes.length,i=0;i<nodeCount;i++){var nodeDef=view.def.nodes[i];if(nodeDef.flags&queryFlags)switch(Services.setCurrentNode(view,nodeDef.index),action){case QueryAction.CheckAndUpdate:checkAndUpdateQuery(view,nodeDef);break;case QueryAction.CheckNoChanges:checkNoChangesQuery(view,nodeDef)}else 0===(nodeDef.childFlags&queryFlags)&&(i+=nodeDef.childCount)}}function attachEmbeddedView(elementData,viewIndex,view){var embeddedViews=elementData.embeddedViews;null==viewIndex&&(viewIndex=embeddedViews.length),addToArray$1(embeddedViews,viewIndex,view);var dvcElementData=declaredViewContainer(view);if(dvcElementData&&dvcElementData!==elementData){var projectedViews=dvcElementData.projectedViews;projectedViews||(projectedViews=dvcElementData.projectedViews=[]),projectedViews.push(view)}for(var queryId in view.def.nodeMatchedQueries)dirtyParentQuery(queryId,view);var prevView=viewIndex>0?embeddedViews[viewIndex-1]:null;renderAttachEmbeddedView(elementData,prevView,view)}function detachEmbeddedView(elementData,viewIndex){var embeddedViews=elementData.embeddedViews;null==viewIndex&&(viewIndex=embeddedViews.length);var view=embeddedViews[viewIndex];removeFromArray(embeddedViews,viewIndex);var dvcElementData=declaredViewContainer(view);if(dvcElementData&&dvcElementData!==elementData){var projectedViews=dvcElementData.projectedViews;removeFromArray(projectedViews,projectedViews.indexOf(view))}for(var queryId in view.def.nodeMatchedQueries)dirtyParentQuery(queryId,view);return renderDetachEmbeddedView(elementData,view),view}function moveEmbeddedView(elementData,oldViewIndex,newViewIndex){var embeddedViews=elementData.embeddedViews,view=embeddedViews[oldViewIndex];removeFromArray(embeddedViews,oldViewIndex),null==newViewIndex&&(newViewIndex=embeddedViews.length),addToArray$1(embeddedViews,newViewIndex,view);for(var queryId in view.def.nodeMatchedQueries)dirtyParentQuery(queryId,view);renderDetachEmbeddedView(elementData,view);var prevView=newViewIndex>0?embeddedViews[newViewIndex-1]:null;return renderAttachEmbeddedView(elementData,prevView,view),view}function renderAttachEmbeddedView(elementData,prevView,view){var prevRenderNode=prevView?renderNode(prevView,prevView.def.lastRootNode):elementData.renderElement,parentNode=view.root.renderer.parentNode(prevRenderNode),nextSibling=view.root.renderer.nextSibling(prevRenderNode);visitRootRenderNodes(view,RenderNodeAction.InsertBefore,parentNode,nextSibling,void 0)}function renderDetachEmbeddedView(elementData,view){var parentNode=view.root.renderer.parentNode(elementData.renderElement);visitRootRenderNodes(view,RenderNodeAction.RemoveChild,parentNode,null,void 0)}function addToArray$1(arr,index,value){index>=arr.length?arr.push(value):arr.splice(index,0,value)}function removeFromArray(arr,index){index>=arr.length-1?arr.pop():arr.splice(index,1)}function initServicesIfNeeded(){if(!initialized){initialized=!0;var services=isDevMode()?createDebugServices():createProdServices();Services.setCurrentNode=services.setCurrentNode,Services.createRootView=services.createRootView,Services.createEmbeddedView=services.createEmbeddedView,Services.checkAndUpdateView=services.checkAndUpdateView,Services.checkNoChangesView=services.checkNoChangesView,Services.destroyView=services.destroyView,Services.attachEmbeddedView=services.attachEmbeddedView,Services.detachEmbeddedView=services.detachEmbeddedView,Services.moveEmbeddedView=services.moveEmbeddedView,Services.resolveDep=services.resolveDep,Services.createDebugContext=services.createDebugContext,Services.handleEvent=services.handleEvent,Services.updateView=services.updateView}}function createProdServices(){return{setCurrentNode:function(){},createRootView:createProdRootView,createEmbeddedView:createEmbeddedView,checkAndUpdateView:checkAndUpdateView,checkNoChangesView:checkNoChangesView,destroyView:destroyView,attachEmbeddedView:attachEmbeddedView,detachEmbeddedView:detachEmbeddedView,moveEmbeddedView:moveEmbeddedView,resolveDep:resolveDep,createDebugContext:function(view,nodeIndex){return new DebugContext_(view,nodeIndex)},handleEvent:function(view,nodeIndex,eventName,event){return view.def.handleEvent(view,nodeIndex,eventName,event)},updateView:function(check,view){return view.def.update(check,view)}}}function createDebugServices(){return{setCurrentNode:debugSetCurrentNode,createRootView:debugCreateRootView,createEmbeddedView:debugCreateEmbeddedView,checkAndUpdateView:debugCheckAndUpdateView,checkNoChangesView:debugCheckNoChangesView,destroyView:debugDestroyView,attachEmbeddedView:attachEmbeddedView,detachEmbeddedView:detachEmbeddedView,moveEmbeddedView:moveEmbeddedView,resolveDep:resolveDep,createDebugContext:function(view,nodeIndex){return new DebugContext_(view,nodeIndex)},handleEvent:debugHandleEvent,updateView:debugUpdateView}}function createProdRootView(injector,projectableNodes,rootSelectorOrNode,def,context){return createRootView(createRootData(injector,projectableNodes,rootSelectorOrNode),def,context)}function debugCreateRootView(injector,projectableNodes,rootSelectorOrNode,def,context){var root=createRootData(injector,projectableNodes,rootSelectorOrNode),debugRoot={injector:root.injector,projectableNodes:root.projectableNodes,selectorOrNode:root.selectorOrNode,renderer:new DebugRenderer(root.renderer),sanitizer:root.sanitizer};return callWithDebugContext("create",createRootView,null,[debugRoot,def,context])}function createRootData(injector,projectableNodes,rootSelectorOrNode){var sanitizer=injector.get(Sanitizer),renderer=isDevMode()?new LegacyRendererAdapter(injector.get(RootRenderer)):new DirectDomRenderer;rootSelectorOrNode?renderer.selectRootElement(rootSelectorOrNode):void 0;return{injector:injector,projectableNodes:projectableNodes,selectorOrNode:rootSelectorOrNode,sanitizer:sanitizer,renderer:renderer}}function debugCreateEmbeddedView(parent,anchorDef,context){return callWithDebugContext("create",createEmbeddedView,null,[parent,anchorDef,context])}function debugCheckAndUpdateView(view){return callWithDebugContext("detectChanges",checkAndUpdateView,null,[view])}function debugCheckNoChangesView(view){return callWithDebugContext("checkNoChanges",checkNoChangesView,null,[view])}function debugDestroyView(view){return callWithDebugContext("destroyView",destroyView,null,[view])}function debugSetCurrentNode(view,nodeIndex){_currentView=view,_currentNodeIndex=nodeIndex}function debugHandleEvent(view,nodeIndex,eventName,event){if(view.state&ViewState.Destroyed)throw viewDestroyedError$1(_currentAction);return debugSetCurrentNode(view,nodeIndex),callWithDebugContext("handleEvent",view.def.handleEvent,null,[view,nodeIndex,eventName,event])}function debugUpdateView(check,view){function debugCheckFn(view,nodeIndex,argStyle,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9){for(var values=argStyle===ArgumentType.Dynamic?v0:[].slice.call(arguments,3),nodeDef=view.def.nodes[nodeIndex],i=0;i<nodeDef.bindings.length;i++){var binding=nodeDef.bindings[i],value=values[i];if((binding.type===BindingType.ElementProperty||binding.type===BindingType.ProviderProperty)&&checkBinding$1(view,nodeDef,i,value)){var elIndex=nodeDef.type===NodeType.Provider?nodeDef.parent:nodeDef.index;setBindingDebugInfo$1(view.root.renderer,asElementData(view,elIndex).renderElement,binding.nonMinifiedName,value)}}var result=check(view,nodeIndex,argStyle,v0,v1,v2,v3,v4,v5,v6,v7,v8,v9);return debugSetCurrentNode(view,nextNodeIndexWithBinding(view,nodeIndex)),result}if(view.state&ViewState.Destroyed)throw viewDestroyedError$1(_currentAction);return debugSetCurrentNode(view,nextNodeIndexWithBinding(view,0)),view.def.update(debugCheckFn,view)}function setBindingDebugInfo$1(renderer,renderNode,propName,value){var renderName="ng-reflect-"+camelCaseToDashCase$1(propName);if(value)try{renderer.setBindingDebugInfo(renderNode,renderName,value.toString())}catch(e){renderer.setBindingDebugInfo(renderNode,renderName,"[ERROR] Exception while trying to serialize the value")}else renderer.removeBindingDebugInfo(renderNode,renderName)}function camelCaseToDashCase$1(input){return input.replace(CAMEL_CASE_REGEXP$1,function(){for(var m=[],_i=0;_i<arguments.length;_i++)m[_i]=arguments[_i];return"-"+m[1].toLowerCase()})}function nextNodeIndexWithBinding(view,nodeIndex){for(var i=nodeIndex;i<view.def.nodes.length;i++){var nodeDef=view.def.nodes[i];if(nodeDef.bindings&&nodeDef.bindings.length)return i}}function findHostElement(view){for(;view&&!isComponentView(view);)view=view.parent;if(view.parent){var hostData=asElementData(view.parent,view.parentIndex);return hostData}}function collectReferences(view,nodeDef,references){for(var queryId in nodeDef.matchedQueries)queryIdIsReference(queryId)&&(references[queryId.slice(1)]=getQueryValue(view,nodeDef,queryId))}function callWithDebugContext(action,fn,self,args){var oldAction=_currentAction,oldView=_currentView,oldNodeIndex=_currentNodeIndex;try{_currentAction=action;var result=fn.apply(self,args);return _currentView=oldView,_currentNodeIndex=oldNodeIndex,_currentAction=oldAction,result}catch(e){if(isViewDebugError(e)||!_currentView)throw e;throw viewWrappedDebugError(e,getCurrentDebugContext())}}function getCurrentDebugContext(){return new DebugContext_(_currentView,_currentNodeIndex)}function _iterableDiffersFactory(){return defaultIterableDiffers}function _keyValueDiffersFactory(){return defaultKeyValueDiffers}function _localeFactory(locale){return locale||"en-US"}function _initViewEngine(){initServicesIfNeeded()}function animate(timing,styles){void 0===styles&&(styles=null);var stylesEntry=styles;if(!isPresent(stylesEntry)){var EMPTY_STYLE={};stylesEntry=new AnimationStyleMetadata([EMPTY_STYLE],1)}return new AnimationAnimateMetadata(timing,stylesEntry)}function group(steps){return new AnimationGroupMetadata(steps)}function sequence(steps){return new AnimationSequenceMetadata(steps)}function style(tokens){var input,offset=null;return"string"==typeof tokens?input=[tokens]:(input=Array.isArray(tokens)?tokens:[tokens],input.forEach(function(entry){var entryOffset=entry.offset;isPresent(entryOffset)&&(offset=null==offset?parseFloat(entryOffset):offset)})),new AnimationStyleMetadata(input,offset)}function state(stateNameExpr,styles){return new AnimationStateDeclarationMetadata(stateNameExpr,styles)}function keyframes(steps){return new AnimationKeyframesSequenceMetadata(steps)}function transition(stateChangeExpr,steps){var animationData=Array.isArray(steps)?new AnimationSequenceMetadata(steps):steps;return new AnimationStateTransitionMetadata(stateChangeExpr,animationData)}function trigger(name,animation){return new AnimationEntryMetadata(name,animation)}function prepareFinalAnimationStyles(previousStyles,newStyles,nullValue){void 0===nullValue&&(nullValue=null);var finalStyles={};return Object.keys(newStyles).forEach(function(prop){var value=newStyles[prop];finalStyles[prop]=value==AUTO_STYLE?nullValue:value.toString()}),Object.keys(previousStyles).forEach(function(prop){isPresent(finalStyles[prop])||(finalStyles[prop]=nullValue)}),finalStyles}function balanceAnimationKeyframes(collectedStyles,finalStateStyles,keyframes){var limit=keyframes.length-1,firstKeyframe=keyframes[0],flatenedFirstKeyframeStyles=flattenStyles(firstKeyframe.styles.styles),extraFirstKeyframeStyles={},hasExtraFirstStyles=!1;Object.keys(collectedStyles).forEach(function(prop){var value=collectedStyles[prop];flatenedFirstKeyframeStyles[prop]||(flatenedFirstKeyframeStyles[prop]=value,extraFirstKeyframeStyles[prop]=value,hasExtraFirstStyles=!0)});var keyframeCollectedStyles=StringMapWrapper.merge({},flatenedFirstKeyframeStyles),finalKeyframe=keyframes[limit];finalKeyframe.styles.styles.unshift(finalStateStyles);var flatenedFinalKeyframeStyles=flattenStyles(finalKeyframe.styles.styles),extraFinalKeyframeStyles={},hasExtraFinalStyles=!1;return Object.keys(keyframeCollectedStyles).forEach(function(prop){isPresent(flatenedFinalKeyframeStyles[prop])||(extraFinalKeyframeStyles[prop]=AUTO_STYLE,hasExtraFinalStyles=!0)}),hasExtraFinalStyles&&finalKeyframe.styles.styles.push(extraFinalKeyframeStyles),Object.keys(flatenedFinalKeyframeStyles).forEach(function(prop){isPresent(flatenedFirstKeyframeStyles[prop])||(extraFirstKeyframeStyles[prop]=AUTO_STYLE,hasExtraFirstStyles=!0)}),hasExtraFirstStyles&&firstKeyframe.styles.styles.push(extraFirstKeyframeStyles),collectAndResolveStyles(collectedStyles,[finalStateStyles]),keyframes}function clearStyles(styles){var finalStyles={};return Object.keys(styles).forEach(function(key){finalStyles[key]=null}),finalStyles}function collectAndResolveStyles(collection,styles){return styles.map(function(entry){var stylesObj={};return Object.keys(entry).forEach(function(prop){var value=entry[prop];value==FILL_STYLE_FLAG&&(value=collection[prop],isPresent(value)||(value=AUTO_STYLE)),collection[prop]=value,stylesObj[prop]=value}),stylesObj})}function renderStyles(element,renderer,styles){Object.keys(styles).forEach(function(prop){renderer.setElementStyle(element,prop,styles[prop])})}function flattenStyles(styles){var finalStyles={};return styles.forEach(function(entry){Object.keys(entry).forEach(function(prop){finalStyles[prop]=entry[prop]})}),finalStyles}function _recursePlayers(player,collectedPlayers){player instanceof AnimationGroupPlayer||player instanceof AnimationSequencePlayer?player.players.forEach(function(player){return _recursePlayers(player,collectedPlayers)}):collectedPlayers.push(player)}var globalScope,__extends=this&&this.__extends||function(d,b){function __(){this.constructor=d}for(var p in b)b.hasOwnProperty(p)&&(d[p]=b[p]);d.prototype=null===b?Object.create(b):(__.prototype=b.prototype,new __)},OpaqueToken=function(){function OpaqueToken(_desc){this._desc=_desc}return OpaqueToken.prototype.toString=function(){return"Token "+this._desc},OpaqueToken}(),InjectionToken=function(_super){function InjectionToken(desc){return _super.call(this,desc)||this}return __extends(InjectionToken,_super),InjectionToken.prototype.toString=function(){return"InjectionToken "+this._desc},InjectionToken}(OpaqueToken);globalScope="undefined"==typeof window?"undefined"!=typeof WorkerGlobalScope&&self instanceof WorkerGlobalScope?self:global:window;var global$1=globalScope;global$1.assert=function(condition){};var _symbolIterator=null,_nextClassId=0,Reflect=global$1.Reflect,ANALYZE_FOR_ENTRY_COMPONENTS=new InjectionToken("AnalyzeForEntryComponents"),Attribute=makeParamDecorator("Attribute",[["attributeName",void 0]]),Query=function(){function Query(){}return Query}(),ContentChildren=makePropDecorator("ContentChildren",[["selector",void 0],{first:!1,isViewQuery:!1,descendants:!1,read:void 0}],Query),ContentChild=makePropDecorator("ContentChild",[["selector",void 0],{first:!0,isViewQuery:!1,descendants:!0,read:void 0}],Query),ViewChildren=makePropDecorator("ViewChildren",[["selector",void 0],{first:!1,isViewQuery:!0,descendants:!0,read:void 0}],Query),ViewChild=makePropDecorator("ViewChild",[["selector",void 0],{first:!0,isViewQuery:!0,descendants:!0,read:void 0}],Query),ChangeDetectionStrategy={};ChangeDetectionStrategy.OnPush=0,ChangeDetectionStrategy.Default=1,ChangeDetectionStrategy[ChangeDetectionStrategy.OnPush]="OnPush",ChangeDetectionStrategy[ChangeDetectionStrategy.Default]="Default";var ChangeDetectorStatus={};ChangeDetectorStatus.CheckOnce=0,ChangeDetectorStatus.Checked=1,ChangeDetectorStatus.CheckAlways=2,ChangeDetectorStatus.Detached=3,ChangeDetectorStatus.Errored=4,ChangeDetectorStatus.Destroyed=5,ChangeDetectorStatus[ChangeDetectorStatus.CheckOnce]="CheckOnce",ChangeDetectorStatus[ChangeDetectorStatus.Checked]="Checked",ChangeDetectorStatus[ChangeDetectorStatus.CheckAlways]="CheckAlways",ChangeDetectorStatus[ChangeDetectorStatus.Detached]="Detached",ChangeDetectorStatus[ChangeDetectorStatus.Errored]="Errored",ChangeDetectorStatus[ChangeDetectorStatus.Destroyed]="Destroyed";var Directive=makeDecorator("Directive",{selector:void 0,inputs:void 0,outputs:void 0,host:void 0,providers:void 0,exportAs:void 0,queries:void 0}),Component=makeDecorator("Component",{selector:void 0,inputs:void 0,outputs:void 0,host:void 0,exportAs:void 0,moduleId:void 0,providers:void 0,viewProviders:void 0,changeDetection:ChangeDetectionStrategy.Default,queries:void 0,templateUrl:void 0,template:void 0,styleUrls:void 0,styles:void 0,animations:void 0,encapsulation:void 0,interpolation:void 0,entryComponents:void 0},Directive),Pipe=makeDecorator("Pipe",{name:void 0,pure:!0}),Input=makePropDecorator("Input",[["bindingPropertyName",void 0]]),Output=makePropDecorator("Output",[["bindingPropertyName",void 0]]),HostBinding=makePropDecorator("HostBinding",[["hostPropertyName",void 0]]),HostListener=makePropDecorator("HostListener",[["eventName",void 0],["args",[]]]),LifecycleHooks={};LifecycleHooks.OnInit=0,LifecycleHooks.OnDestroy=1,LifecycleHooks.DoCheck=2,LifecycleHooks.OnChanges=3,LifecycleHooks.AfterContentInit=4,LifecycleHooks.AfterContentChecked=5,LifecycleHooks.AfterViewInit=6,LifecycleHooks.AfterViewChecked=7,LifecycleHooks[LifecycleHooks.OnInit]="OnInit",LifecycleHooks[LifecycleHooks.OnDestroy]="OnDestroy",LifecycleHooks[LifecycleHooks.DoCheck]="DoCheck",LifecycleHooks[LifecycleHooks.OnChanges]="OnChanges",LifecycleHooks[LifecycleHooks.AfterContentInit]="AfterContentInit",LifecycleHooks[LifecycleHooks.AfterContentChecked]="AfterContentChecked",LifecycleHooks[LifecycleHooks.AfterViewInit]="AfterViewInit",LifecycleHooks[LifecycleHooks.AfterViewChecked]="AfterViewChecked";var LIFECYCLE_HOOKS_VALUES=[LifecycleHooks.OnInit,LifecycleHooks.OnDestroy,LifecycleHooks.DoCheck,LifecycleHooks.OnChanges,LifecycleHooks.AfterContentInit,LifecycleHooks.AfterContentChecked,LifecycleHooks.AfterViewInit,LifecycleHooks.AfterViewChecked],OnChanges=function(){function OnChanges(){}return OnChanges.prototype.ngOnChanges=function(changes){},OnChanges}(),OnInit=function(){function OnInit(){}return OnInit.prototype.ngOnInit=function(){},OnInit}(),DoCheck=function(){function DoCheck(){}return DoCheck.prototype.ngDoCheck=function(){},DoCheck}(),OnDestroy=function(){function OnDestroy(){}return OnDestroy.prototype.ngOnDestroy=function(){},OnDestroy}(),AfterContentInit=function(){function AfterContentInit(){}return AfterContentInit.prototype.ngAfterContentInit=function(){},AfterContentInit}(),AfterContentChecked=function(){function AfterContentChecked(){}return AfterContentChecked.prototype.ngAfterContentChecked=function(){},AfterContentChecked}(),AfterViewInit=function(){function AfterViewInit(){}return AfterViewInit.prototype.ngAfterViewInit=function(){},AfterViewInit}(),AfterViewChecked=function(){function AfterViewChecked(){}return AfterViewChecked.prototype.ngAfterViewChecked=function(){},AfterViewChecked}(),CUSTOM_ELEMENTS_SCHEMA={name:"custom-elements"},NO_ERRORS_SCHEMA={name:"no-errors-schema"},NgModule=makeDecorator("NgModule",{providers:void 0,declarations:void 0,imports:void 0,exports:void 0,entryComponents:void 0,bootstrap:void 0,schemas:void 0,id:void 0}),ViewEncapsulation={};ViewEncapsulation.Emulated=0,ViewEncapsulation.Native=1,ViewEncapsulation.None=2,
ViewEncapsulation[ViewEncapsulation.Emulated]="Emulated",ViewEncapsulation[ViewEncapsulation.Native]="Native",ViewEncapsulation[ViewEncapsulation.None]="None";var ViewMetadata=function(){function ViewMetadata(_a){var _b=void 0===_a?{}:_a,templateUrl=_b.templateUrl,template=_b.template,encapsulation=_b.encapsulation,styles=_b.styles,styleUrls=_b.styleUrls,animations=_b.animations,interpolation=_b.interpolation;this.templateUrl=templateUrl,this.template=template,this.styleUrls=styleUrls,this.styles=styles,this.encapsulation=encapsulation,this.animations=animations,this.interpolation=interpolation}return ViewMetadata}(),Version=function(){function Version(full){this.full=full}return Object.defineProperty(Version.prototype,"major",{get:function(){return this.full.split(".")[0]},enumerable:!0,configurable:!0}),Object.defineProperty(Version.prototype,"minor",{get:function(){return this.full.split(".")[1]},enumerable:!0,configurable:!0}),Object.defineProperty(Version.prototype,"patch",{get:function(){return this.full.split(".").slice(2).join(".")},enumerable:!0,configurable:!0}),Version}(),VERSION=new Version("4.0.0-beta.7-1dc9be4"),Inject=makeParamDecorator("Inject",[["token",void 0]]),Optional=makeParamDecorator("Optional",[]),Injectable=makeDecorator("Injectable",[]),Self=makeParamDecorator("Self",[]),SkipSelf=makeParamDecorator("SkipSelf",[]),Host=makeParamDecorator("Host",[]),_THROW_IF_NOT_FOUND=new Object,THROW_IF_NOT_FOUND=_THROW_IF_NOT_FOUND,_NullInjector=function(){function _NullInjector(){}return _NullInjector.prototype.get=function(token,notFoundValue){if(void 0===notFoundValue&&(notFoundValue=_THROW_IF_NOT_FOUND),notFoundValue===_THROW_IF_NOT_FOUND)throw new Error("No provider for "+stringify(token)+"!");return notFoundValue},_NullInjector}(),Injector=function(){function Injector(){}return Injector.prototype.get=function(token,notFoundValue){},Injector.prototype.get=function(token,notFoundValue){},Injector}();Injector.THROW_IF_NOT_FOUND=_THROW_IF_NOT_FOUND,Injector.NULL=new _NullInjector;/**
     * @license
...
var __extends$6=this&&this.__extends||function(d,b){function __(){this.constructor=d}for(var p in b)b.hasOwnProperty(p)&&(d[p]=b[p]);d.prototype=null===b?Object.create(b):(__.prototype=b.prototype,new __)},TemplateRef=function(){function TemplateRef(){}return TemplateRef.prototype.elementRef=function(){},TemplateRef.prototype.createEmbeddedView=function(context){},TemplateRef}(),TemplateRef_=function(_super){function TemplateRef_(_parentView,_nodeIndex,_nativeElement){var _this=_super.call(this)||this;return _this._parentView=_parentView,_this._nodeIndex=_nodeIndex,_this._nativeElement=_nativeElement,_this}return __extends$6(TemplateRef_,_super),TemplateRef_.prototype.createEmbeddedView=function(context){var view=this._parentView.createEmbeddedViewInternal(this._nodeIndex);return view.create(context||{}),view.ref},Object.defineProperty(TemplateRef_.prototype,"elementRef",{get:function(){return new ElementRef(this._nativeElement)},enumerable:!0,configurable:!0}),TemplateRef_}(TemplateRef),ViewContainerRef=function(){function ViewContainerRef(){}return ViewContainerRef.prototype.element=function(){},ViewContainerRef.prototype.injector=function(){},ViewContainerRef.prototype.parentInjector=function(){},ViewContainerRef.prototype.clear=function(){},ViewContainerRef.prototype.get=function(index){},ViewContainerRef.prototype.length=function(){},ViewContainerRef.prototype.createEmbeddedView=function(templateRef,context,index){},ViewContainerRef.prototype.createComponent=function(componentFactory,index,injector,projectableNodes){},ViewContainerRef.prototype.insert=function(viewRef,index){},ViewContainerRef.prototype.move=function(viewRef,currentIndex){},ViewContainerRef.prototype.indexOf=function(viewRef){},ViewContainerRef.prototype.remove=function(index){},ViewContainerRef.prototype.detach=function(index){},ViewContainerRef}(),ViewContainerRef_=function(){function ViewContainerRef_(_element){this._element=_element,this._createComponentInContainerScope=wtfCreateScope("ViewContainerRef#createComponent()"),this._insertScope=wtfCreateScope("ViewContainerRef#insert()"),this._removeScope=wtfCreateScope("ViewContainerRef#remove()"),this._detachScope=wtfCreateScope("ViewContainerRef#detach()")}return ViewContainerRef_.prototype.get=function(index){return this._element.nestedViews[index].ref},Object.defineProperty(ViewContainerRef_.prototype,"length",{get:function(){var views=this._element.nestedViews;return isPresent(views)?views.length:0},enumerable:!0,configurable:!0}),Object.defineProperty(ViewContainerRef_.prototype,"element",{get:function(){return this._element.elementRef},enumerable:!0,configurable:!0}),Object.defineProperty(ViewContainerRef_.prototype,"injector",{get:function(){return this._element.injector},enumerable:!0,configurable:!0}),Object.defineProperty(ViewContainerRef_.prototype,"parentInjector",{get:function(){return this._element.parentInjector},enumerable:!0,configurable:!0}),ViewContainerRef_.prototype.createEmbeddedView=function(templateRef,context,index){void 0===context&&(context=null),void 0===index&&(index=-1);var viewRef=templateRef.createEmbeddedView(context);return this.insert(viewRef,index),viewRef},ViewContainerRef_.prototype.createComponent=function(componentFactory,index,injector,projectableNodes){void 0===index&&(index=-1),void 0===injector&&(injector=null),void 0===projectableNodes&&(projectableNodes=null);var s=this._createComponentInContainerScope(),contextInjector=injector||this._element.parentInjector,componentRef=componentFactory.create(contextInjector,projectableNodes);return this.insert(componentRef.hostView,index),wtfLeave(s,componentRef)},ViewContainerRef_.prototype.insert=function(viewRef,index){void 0===index&&(index=-1);var s=this._insertScope();index==-1&&(index=this.length);var viewRef_=viewRef;return this._element.attachView(viewRef_.internalView,index),wtfLeave(s,viewRef_)},ViewContainerRef_.prototype.move=function(viewRef,currentIndex){var s=this._insertScope();if(currentIndex!=-1){var viewRef_=viewRef;return this._element.moveView(viewRef_.internalView,currentIndex),wtfLeave(s,viewRef_)}},ViewContainerRef_.prototype.indexOf=function(viewRef){return this.length?this._element.nestedViews.indexOf(viewRef.internalView):-1},ViewContainerRef_.prototype.remove=function(index){void 0===index&&(index=-1);var s=this._removeScope();index==-1&&(index=this.length-1);var view=this._element.detachView(index);view.destroy(),wtfLeave(s)},ViewContainerRef_.prototype.detach=function(index){void 0===index&&(index=-1);var s=this._detachScope();index==-1&&(index=this.length-1);var view=this._element.detachView(index);return wtfLeave(s,view.ref)},ViewContainerRef_.prototype.clear=function(){for(var i=this.length-1;i>=0;i--)this.remove(i)},ViewContainerRef_}(),__extends$7=this&&this.__extends||function(d,b){function __(){this.constructor=d}for(var p in b)b.hasOwnProperty(p)&&(d[p]=b[p]);d.prototype=null===b?Object.create(b):(__.prototype=b.prototype,new __)},ViewRef=function(_super){function ViewRef(){return null!==_super&&_super.apply(this,arguments)||this}return __extends$7(ViewRef,_super),ViewRef.prototype.destroy=function(){},ViewRef.prototype.destroyed=function(){},ViewRef.prototype.onDestroy=function(callback){},ViewRef}(ChangeDetectorRef),EmbeddedViewRef=function(_super){function EmbeddedViewRef(){return null!==_super&&_super.apply(this,arguments)||this}return __extends$7(EmbeddedViewRef,_super),EmbeddedViewRef.prototype.context=function(){},EmbeddedViewRef.prototype.rootNodes=function(){},EmbeddedViewRef}(ViewRef),ViewRef_=function(){function ViewRef_(_view,animationQueue){this._view=_view,this.animationQueue=animationQueue,this._view=_view,this._originalMode=this._view.cdMode}return Object.defineProperty(ViewRef_.prototype,"internalView",{get:function(){return this._view},enumerable:!0,configurable:!0}),Object.defineProperty(ViewRef_.prototype,"rootNodes",{get:function(){return this._view.flatRootNodes},enumerable:!0,configurable:!0}),Object.defineProperty(ViewRef_.prototype,"context",{get:function(){return this._view.context},enumerable:!0,configurable:!0}),Object.defineProperty(ViewRef_.prototype,"destroyed",{get:function(){return this._view.destroyed},enumerable:!0,configurable:!0}),ViewRef_.prototype.markForCheck=function(){this._view.markPathToRootAsCheckOnce()},ViewRef_.prototype.detach=function(){this._view.cdMode=ChangeDetectorStatus.Detached},ViewRef_.prototype.detectChanges=function(){this._view.detectChanges(!1),this.animationQueue.flush()},ViewRef_.prototype.checkNoChanges=function(){this._view.detectChanges(!0)},ViewRef_.prototype.reattach=function(){this._view.cdMode=this._originalMode,this.markForCheck()},ViewRef_.prototype.onDestroy=function(callback){this._view.disposables||(this._view.disposables=[]),this._view.disposables.push(callback)},ViewRef_.prototype.destroy=function(){this._view.detachAndDestroy()},ViewRef_}(),__extends$8=this&&this.__extends||function(d,b){function __(){this.constructor=d}for(var p in b)b.hasOwnProperty(p)&&(d[p]=b[p]);d.prototype=null===b?Object.create(b):(__.prototype=b.prototype,new __)},EventListener=function(){function EventListener(name,callback){this.name=name,this.callback=callback}return EventListener}(),DebugNode=function(){function DebugNode(nativeNode,parent,_debugInfo){this._debugInfo=_debugInfo,this.nativeNode=nativeNode,parent&&parent instanceof DebugElement?parent.addChild(this):this.parent=null,this.listeners=[]}return Object.defineProperty(DebugNode.prototype,"injector",{get:function(){return this._debugInfo?this._debugInfo.injector:null},enumerable:!0,configurable:!0}),Object.defineProperty(DebugNode.prototype,"componentInstance",{get:function(){return this._debugInfo?this._debugInfo.component:null},enumerable:!0,configurable:!0}),Object.defineProperty(DebugNode.prototype,"context",{get:function(){return this._debugInfo?this._debugInfo.context:null},enumerable:!0,configurable:!0}),Object.defineProperty(DebugNode.prototype,"references",{get:function(){return this._debugInfo?this._debugInfo.references:null},enumerable:!0,configurable:!0}),Object.defineProperty(DebugNode.prototype,"providerTokens",{get:function(){return this._debugInfo?this._debugInfo.providerTokens:null},enumerable:!0,configurable:!0}),Object.defineProperty(DebugNode.prototype,"source",{get:function(){return this._debugInfo?this._debugInfo.source:null},enumerable:!0,configurable:!0}),DebugNode}(),DebugElement=function(_super){function DebugElement(nativeNode,parent,_debugInfo){var _this=_super.call(this,nativeNode,parent,_debugInfo)||this;return _this.properties={},_this.attributes={},_this.classes={},_this.styles={},_this.childNodes=[],_this.nativeElement=nativeNode,_this}return __extends$8(DebugElement,_super),DebugElement.prototype.addChild=function(child){child&&(this.childNodes.push(child),child.parent=this)},DebugElement.prototype.removeChild=function(child){var childIndex=this.childNodes.indexOf(child);childIndex!==-1&&(child.parent=null,this.childNodes.splice(childIndex,1))},DebugElement.prototype.insertChildrenAfter=function(child,newChildren){var siblingIndex=this.childNodes.indexOf(child);if(siblingIndex!==-1){var previousChildren=this.childNodes.slice(0,siblingIndex+1),nextChildren=this.childNodes.slice(siblingIndex+1);this.childNodes=previousChildren.concat(newChildren,nextChildren);for(var i=0;i<newChildren.length;++i){var newChild=newChildren[i];newChild.parent&&newChild.parent.removeChild(newChild),newChild.parent=this}}},DebugElement.prototype.query=function(predicate){var results=this.queryAll(predicate);return results[0]||null},DebugElement.prototype.queryAll=function(predicate){var matches=[];return _queryElementChildren(this,predicate,matches),matches},DebugElement.prototype.queryAllNodes=function(predicate){var matches=[];return _queryNodeChildren(this,predicate,matches),matches},Object.defineProperty(DebugElement.prototype,"children",{get:function(){return this.childNodes.filter(function(node){return node instanceof DebugElement})},enumerable:!0,configurable:!0}),DebugElement.prototype.triggerEventHandler=function(eventName,eventObj){this.listeners.forEach(function(listener){listener.name==eventName&&listener.callback(eventObj)})},DebugElement}(DebugNode),_nativeNodeToDebugNode=new Map,AnimationPlayer_=function(){function AnimationPlayer(){}return AnimationPlayer.prototype.onDone=function(fn){},AnimationPlayer.prototype.onStart=function(fn){},AnimationPlayer.prototype.onDestroy=function(fn){},AnimationPlayer.prototype.init=function(){},AnimationPlayer.prototype.hasStarted=function(){},AnimationPlayer.prototype.play=function(){},AnimationPlayer.prototype.pause=function(){},AnimationPlayer.prototype.restart=function(){},AnimationPlayer.prototype.finish=function(){},AnimationPlayer.prototype.destroy=function(){},AnimationPlayer.prototype.reset=function(){},AnimationPlayer.prototype.setPosition=function(p){},AnimationPlayer.prototype.getPosition=function(){},Object.defineProperty(AnimationPlayer.prototype,"parentPlayer",{get:function(){throw new Error("NOT IMPLEMENTED: Base Class")},set:function(player){throw new Error("NOT IMPLEMENTED: Base Class")},enumerable:!0,configurable:!0}),AnimationPlayer}(),NoOpAnimationPlayer=function(){function NoOpAnimationPlayer(){var _this=this;this._onDoneFns=[],this._onStartFns=[],this._onDestroyFns=[],this._started=!1,this._destroyed=!1,this._finished=!1,this.parentPlayer=null,scheduleMicroTask(function(){return _this._onFinish()})}return NoOpAnimationPlayer.prototype._onFinish=function(){this._finished||(this._finished=!0,this._onDoneFns.forEach(function(fn){return fn()}),this._onDoneFns=[])},NoOpAnimationPlayer.prototype.onStart=function(fn){this._onStartFns.push(fn)},NoOpAnimationPlayer.prototype.onDone=function(fn){this._onDoneFns.push(fn)},NoOpAnimationPlayer.prototype.onDestroy=function(fn){this._onDestroyFns.push(fn)},NoOpAnimationPlayer.prototype.hasStarted=function(){return this._started},NoOpAnimationPlayer.prototype.init=function(){},NoOpAnimationPlayer.prototype.play=function(){this.hasStarted()||(this._onStartFns.forEach(function(fn){return fn()}),this._onStartFns=[]),this._started=!0},NoOpAnimationPlayer.prototype.pause=function(){},NoOpAnimationPlayer.prototype.restart=function(){},NoOpAnimationPlayer.prototype.finish=function(){this._onFinish()},NoOpAnimationPlayer.prototype.destroy=function(){this._destroyed||(this._destroyed=!0,this.finish(),this._onDestroyFns.forEach(function(fn){return fn()}),this._onDestroyFns=[])},NoOpAnimationPlayer.prototype.reset=function(){},NoOpAnimationPlayer.prototype.setPosition=function(p){},NoOpAnimationPlayer.prototype.getPosition=function(){return 0},NoOpAnimationPlayer}(),__extends$9=this&&this.__extends||function(d,b){function __(){this.constructor=d}for(var p in b)b.hasOwnProperty(p)&&(d[p]=b[p]);d.prototype=null===b?Object.create(b):(__.prototype=b.prototype,new __)},TransitionEngine=function(){function TransitionEngine(){}return TransitionEngine.prototype.insertNode=function(container,element){},TransitionEngine.prototype.removeNode=function(element){},TransitionEngine.prototype.process=function(element,instructions){},TransitionEngine.prototype.triggerAnimations=function(){},TransitionEngine}(),NoOpTransitionEngine=function(_super){function NoOpTransitionEngine(){return _super.call(this)||this}return __extends$9(NoOpTransitionEngine,_super),NoOpTransitionEngine.prototype.insertNode=function(container,element){container.appendChild(element)},NoOpTransitionEngine.prototype.removeNode=function(element){remove(element)},NoOpTransitionEngine.prototype.process=function(element,instructions){return new NoOpAnimationPlayer},NoOpTransitionEngine.prototype.triggerAnimations=function(){},NoOpTransitionEngine}(TransitionEngine),_CORE_PLATFORM_PROVIDERS=[PlatformRef_,{provide:PlatformRef,useExisting:PlatformRef_},{provide:Reflector,useFactory:_reflector,deps:[]},{provide:ReflectorReader,useExisting:Reflector},{provide:TransitionEngine,useClass:NoOpTransitionEngine},TestabilityRegistry,Console],platformCore=createPlatformFactory(null,"core",_CORE_PLATFORM_PROVIDERS),LOCALE_ID=new InjectionToken("LocaleId"),TRANSLATIONS=new InjectionToken("Translations"),TRANSLATIONS_FORMAT=new InjectionToken("TranslationsFormat"),MissingTranslationStrategy={};MissingTranslationStrategy.Error=0,MissingTranslationStrategy.Warning=1,MissingTranslationStrategy.Ignore=2,MissingTranslationStrategy[MissingTranslationStrategy.Error]="Error",MissingTranslationStrategy[MissingTranslationStrategy.Warning]="Warning",MissingTranslationStrategy[MissingTranslationStrategy.Ignore]="Ignore";/**
     ...
     * found in the LICENSE file at https://angular.io/license
     */
var __extends$10=this&&this.__extends||function(d,b){function __(){this.constructor=d}for(var p in b)b.hasOwnProperty(p)&&(d[p]=b[p]);d.prototype=null===b?Object.create(b):(__.prototype=b.prototype,new __)},ArgumentType={};ArgumentType.Inline=0,ArgumentType.Dynamic=1,ArgumentType[ArgumentType.Inline]="Inline",ArgumentType[ArgumentType.Dynamic]="Dynamic";var ViewFlags={};ViewFlags.None=0,ViewFlags.OnPush=2,ViewFlags[ViewFlags.None]="None",ViewFlags[ViewFlags.OnPush]="OnPush";var NodeType={};NodeType.Element=0,NodeType.Text=1,NodeType.Provider=2,NodeType.PureExpression=3,NodeType.Query=4,NodeType.NgContent=5,NodeType[NodeType.Element]="Element",NodeType[NodeType.Text]="Text",NodeType[NodeType.Provider]="Provider",NodeType[NodeType.PureExpression]="PureExpression",NodeType[NodeType.Query]="Query",NodeType[NodeType.NgContent]="NgContent";var NodeFlags={};NodeFlags.None=0,NodeFlags.OnInit=1,NodeFlags.OnDestroy=2,NodeFlags.DoCheck=4,NodeFlags.OnChanges=8,NodeFlags.AfterContentInit=16,NodeFlags.AfterContentChecked=32,NodeFlags.AfterViewInit=64,NodeFlags.AfterViewChecked=128,NodeFlags.HasEmbeddedViews=256,NodeFlags.HasComponent=512,NodeFlags.HasContentQuery=1024,NodeFlags.HasViewQuery=2048,NodeFlags.LazyProvider=4096,NodeFlags[NodeFlags.None]="None",NodeFlags[NodeFlags.OnInit]="OnInit",NodeFlags[NodeFlags.OnDestroy]="OnDestroy",NodeFlags[NodeFlags.DoCheck]="DoCheck",NodeFlags[NodeFlags.OnChanges]="OnChanges",NodeFlags[NodeFlags.AfterContentInit]="AfterContentInit",NodeFlags[NodeFlags.AfterContentChecked]="AfterContentChecked",NodeFlags[NodeFlags.AfterViewInit]="AfterViewInit",NodeFlags[NodeFlags.AfterViewChecked]="AfterViewChecked",NodeFlags[NodeFlags.HasEmbeddedViews]="HasEmbeddedViews",NodeFlags[NodeFlags.HasComponent]="HasComponent",NodeFlags[NodeFlags.HasContentQuery]="HasContentQuery",NodeFlags[NodeFlags.HasViewQuery]="HasViewQuery",NodeFlags[NodeFlags.LazyProvider]="LazyProvider";var BindingType={};BindingType.ElementAttribute=0,BindingType.ElementClass=1,BindingType.ElementStyle=2,BindingType.ElementProperty=3,BindingType.ProviderProperty=4,BindingType.Interpolation=5,BindingType.PureExpressionProperty=6,BindingType[BindingType.ElementAttribute]="ElementAttribute",BindingType[BindingType.ElementClass]="ElementClass",BindingType[BindingType.ElementStyle]="ElementStyle",BindingType[BindingType.ElementProperty]="ElementProperty",BindingType[BindingType.ProviderProperty]="ProviderProperty",BindingType[BindingType.Interpolation]="Interpolation",BindingType[BindingType.PureExpressionProperty]="PureExpressionProperty";var QueryValueType={};QueryValueType.ElementRef=0,QueryValueType.RenderElement=1,QueryValueType.TemplateRef=2,QueryValueType.ViewContainerRef=3,QueryValueType.Provider=4,QueryValueType[QueryValueType.ElementRef]="ElementRef",QueryValueType[QueryValueType.RenderElement]="RenderElement",QueryValueType[QueryValueType.TemplateRef]="TemplateRef",QueryValueType[QueryValueType.ViewContainerRef]="ViewContainerRef",QueryValueType[QueryValueType.Provider]="Provider";var ProviderType={};ProviderType.Value=0,ProviderType.Class=1,ProviderType.Factory=2,ProviderType.UseExisting=3,ProviderType[ProviderType.Value]="Value",ProviderType[ProviderType.Class]="Class",ProviderType[ProviderType.Factory]="Factory",ProviderType[ProviderType.UseExisting]="UseExisting";var DepFlags={};DepFlags.None=0,DepFlags.SkipSelf=1,DepFlags.Optional=2,DepFlags.Value=8,DepFlags[DepFlags.None]="None",DepFlags[DepFlags.SkipSelf]="SkipSelf",DepFlags[DepFlags.Optional]="Optional",DepFlags[DepFlags.Value]="Value";var PureExpressionType={};PureExpressionType.Array=0,PureExpressionType.Object=1,PureExpressionType.Pipe=2,PureExpressionType[PureExpressionType.Array]="Array",PureExpressionType[PureExpressionType.Object]="Object",PureExpressionType[PureExpressionType.Pipe]="Pipe";var QueryBindingType={};QueryBindingType.First=0,QueryBindingType.All=1,QueryBindingType[QueryBindingType.First]="First",QueryBindingType[QueryBindingType.All]="All";var ViewState={};ViewState.FirstCheck=1,ViewState.ChecksEnabled=2,ViewState.Errored=4,ViewState.Destroyed=8,ViewState[ViewState.FirstCheck]="FirstCheck",ViewState[ViewState.ChecksEnabled]="ChecksEnabled",ViewState[ViewState.Errored]="Errored",ViewState[ViewState.Destroyed]="Destroyed";var NodeData=function(){function NodeData(){}return NodeData}(),RenderDebugContext=function(){function RenderDebugContext(){}return RenderDebugContext.prototype.injector=function(){},RenderDebugContext.prototype.component=function(){},RenderDebugContext.prototype.providerTokens=function(){},RenderDebugContext.prototype.references=function(){},RenderDebugContext.prototype.context=function(){},RenderDebugContext.prototype.source=function(){},RenderDebugContext.prototype.componentRenderElement=function(){},RenderDebugContext.prototype.renderNode=function(){},RenderDebugContext}(),DebugContext=function(_super){function DebugContext(){return null!==_super&&_super.apply(this,arguments)||this}return __extends$10(DebugContext,_super),DebugContext.prototype.view=function(){},DebugContext.prototype.nodeIndex=function(){},DebugContext}(RenderDebugContext),Services={setCurrentNode:void 0,createRootView:void 0,createEmbeddedView:void 0,checkAndUpdateView:void 0,checkNoChangesView:void 0,destroyView:void 0,attachEmbeddedView:void 0,detachEmbeddedView:void 0,moveEmbeddedView:void 0,resolveDep:void 0,createDebugContext:void 0,handleEvent:void 0,updateView:void 0},_tokenKeyCache=new Map,VIEW_DEFINITION_CACHE=new WeakMap,RenderNodeAction={};RenderNodeAction.Collect=0,RenderNodeAction.AppendChild=1,RenderNodeAction.InsertBefore=2,RenderNodeAction.RemoveChild=3,RenderNodeAction[RenderNodeAction.Collect]="Collect",RenderNodeAction[RenderNodeAction.AppendChild]="AppendChild",RenderNodeAction[RenderNodeAction.InsertBefore]="InsertBefore",RenderNodeAction[RenderNodeAction.RemoveChild]="RemoveChild";var EMPTY_CONTEXT=new Object,ComponentFactory_=function(){function ComponentFactory_(selector,componentType,_viewDefFactory){this.selector=selector,this.componentType=componentType,this._viewClass=_viewDefFactory}return ComponentFactory_.prototype.create=function(injector,projectableNodes,rootSelectorOrNode){void 0===projectableNodes&&(projectableNodes=null),void 0===rootSelectorOrNode&&(rootSelectorOrNode=null);for(var componentNodeIndex,viewDef=resolveViewDefinition(this._viewClass),len=viewDef.nodes.length,i=0;i<len;i++){var nodeDef=viewDef.nodes[i];if(nodeDef.provider&&nodeDef.provider.component){componentNodeIndex=i;break}}if(null==componentNodeIndex)throw new Error("Illegal State: Could not find a component in the view definition!");var view=Services.createRootView(injector,projectableNodes||[],rootSelectorOrNode,viewDef,EMPTY_CONTEXT),component=asProviderData(view,componentNodeIndex).instance;return new ComponentRef_$1(view,new ViewRef_$1(view),component)},ComponentFactory_}(),ComponentRef_$1=function(){function ComponentRef_(_view,_viewRef,_component){this._view=_view,this._viewRef=_viewRef,this._component=_component}return Object.defineProperty(ComponentRef_.prototype,"location",{get:function(){return new ElementRef(asElementData(this._view,0).renderElement)},enumerable:!0,configurable:!0}),Object.defineProperty(ComponentRef_.prototype,"injector",{get:function(){return new Injector_(this._view,0)},enumerable:!0,configurable:!0}),Object.defineProperty(ComponentRef_.prototype,"instance",{get:function(){return this._component},enumerable:!0,configurable:!0}),Object.defineProperty(ComponentRef_.prototype,"hostView",{get:function(){return this._viewRef},enumerable:!0,configurable:!0}),Object.defineProperty(ComponentRef_.prototype,"changeDetectorRef",{get:function(){return this._viewRef},enumerable:!0,configurable:!0}),Object.defineProperty(ComponentRef_.prototype,"componentType",{get:function(){return this._component.constructor},enumerable:!0,configurable:!0}),ComponentRef_.prototype.destroy=function(){this._viewRef.destroy()},ComponentRef_.prototype.onDestroy=function(callback){this._viewRef.onDestroy(callback)},ComponentRef_}(),ViewContainerRef_$1=function(){function ViewContainerRef_(_view,_elIndex){this._view=_view,this._elIndex=_elIndex,this._data=asElementData(_view,_elIndex)}return Object.defineProperty(ViewContainerRef_.prototype,"element",{get:function(){return new ElementRef(this._data.renderElement)},enumerable:!0,configurable:!0}),Object.defineProperty(ViewContainerRef_.prototype,"injector",{get:function(){return new Injector_(this._view,this._elIndex)},enumerable:!0,configurable:!0}),Object.defineProperty(ViewContainerRef_.prototype,"parentInjector",{get:function(){for(var view=this._view,elIndex=view.def.nodes[this._elIndex].parent;null==elIndex&&view;)elIndex=viewParentDiIndex(view),view=view.parent;return view?new Injector_(view,elIndex):this._view.root.injector},enumerable:!0,configurable:!0}),ViewContainerRef_.prototype.clear=function(){for(var len=this._data.embeddedViews.length,i=len-1;i>=0;i--){var view=Services.detachEmbeddedView(this._data,i);Services.destroyView(view)}},ViewContainerRef_.prototype.get=function(index){return new ViewRef_$1(this._data.embeddedViews[index])},Object.defineProperty(ViewContainerRef_.prototype,"length",{get:function(){return this._data.embeddedViews.length},enumerable:!0,configurable:!0}),ViewContainerRef_.prototype.createEmbeddedView=function(templateRef,context,index){var viewRef=templateRef.createEmbeddedView(context||{});return this.insert(viewRef,index),viewRef},ViewContainerRef_.prototype.createComponent=function(componentFactory,index,injector,projectableNodes){var contextInjector=injector||this.parentInjector,componentRef=componentFactory.create(contextInjector,projectableNodes);return this.insert(componentRef.hostView,index),componentRef},ViewContainerRef_.prototype.insert=function(viewRef,index){var viewData=viewRef._view;return Services.attachEmbeddedView(this._data,index,viewData),viewRef},ViewContainerRef_.prototype.move=function(viewRef,currentIndex){var previousIndex=this._data.embeddedViews.indexOf(viewRef._view);return Services.moveEmbeddedView(this._data,previousIndex,currentIndex),viewRef},ViewContainerRef_.prototype.indexOf=function(viewRef){return this._data.embeddedViews.indexOf(viewRef._view)},ViewContainerRef_.prototype.remove=function(index){var viewData=Services.detachEmbeddedView(this._data,index);Services.destroyView(viewData)},ViewContainerRef_.prototype.detach=function(index){var view=this.get(index);return Services.detachEmbeddedView(this._data,index),view},ViewContainerRef_}(),ViewRef_$1=function(){function ViewRef_(_view){this._view=_view}return Object.defineProperty(ViewRef_.prototype,"rootNodes",{get:function(){return rootRenderNodes(this._view)},enumerable:!0,configurable:!0}),Object.defineProperty(ViewRef_.prototype,"context",{get:function(){return this._view.context},enumerable:!0,configurable:!0}),Object.defineProperty(ViewRef_.prototype,"destroyed",{get:function(){return 0!==(this._view.state&ViewState.Destroyed)},enumerable:!0,configurable:!0}),ViewRef_.prototype.markForCheck=function(){this.reattach()},ViewRef_.prototype.detach=function(){this._view.state&=~ViewState.ChecksEnabled},ViewRef_.prototype.detectChanges=function(){Services.checkAndUpdateView(this._view)},ViewRef_.prototype.checkNoChanges=function(){Services.checkNoChangesView(this._view)},ViewRef_.prototype.reattach=function(){this._view.state|=ViewState.ChecksEnabled},ViewRef_.prototype.onDestroy=function(callback){this._view.disposables.push(callback)},ViewRef_.prototype.destroy=function(){Services.destroyView(this._view)},ViewRef_}(),TemplateRef_$1=function(){function TemplateRef_(_parentView,_def){this._parentView=_parentView,this._def=_def}return TemplateRef_.prototype.createEmbeddedView=function(context){return new ViewRef_$1(Services.createEmbeddedView(this._parentView,this._def,context))},Object.defineProperty(TemplateRef_.prototype,"elementRef",{get:function(){return new ElementRef(asElementData(this._parentView,this._def.index).renderElement)},enumerable:!0,configurable:!0}),TemplateRef_}(),Injector_=function(){function Injector_(view,elIndex){this.view=view,this.elIndex=elIndex}return Injector_.prototype.get=function(token,notFoundValue){return void 0===notFoundValue&&(notFoundValue=Injector.THROW_IF_NOT_FOUND),Services.resolveDep(this.view,void 0,this.elIndex,{flags:DepFlags.None,token:token,tokenKey:tokenKey(token)},notFoundValue)},Injector_}(),RendererV1TokenKey=tokenKey(Renderer),ElementRefTokenKey=tokenKey(ElementRef),ViewContainerRefTokenKey=tokenKey(ViewContainerRef),TemplateRefTokenKey=tokenKey(TemplateRef),ChangeDetectorRefTokenKey=tokenKey(ChangeDetectorRef),InjectorRefTokenKey=tokenKey(Injector),NOT_CREATED=new Object,DirectDomRenderer=function(){function DirectDomRenderer(){}return DirectDomRenderer.prototype.createElement=function(name){return document.createElement(name)},DirectDomRenderer.prototype.createComment=function(value){return document.createComment(value)},DirectDomRenderer.prototype.createText=function(value){return document.createTextNode(value)},DirectDomRenderer.prototype.appendChild=function(parent,newChild){parent.appendChild(newChild)},DirectDomRenderer.prototype.insertBefore=function(parent,newChild,refChild){parent&&parent.insertBefore(newChild,refChild)},DirectDomRenderer.prototype.removeChild=function(parent,oldChild){parent&&parent.removeChild(oldChild)},DirectDomRenderer.prototype.selectRootElement=function(selectorOrNode,debugInfo){var el;return el="string"==typeof selectorOrNode?document.querySelector(selectorOrNode):selectorOrNode,el.textContent="",el},DirectDomRenderer.prototype.parentNode=function(node){return node.parentNode},DirectDomRenderer.prototype.nextSibling=function(node){return node.nextSiblibng},DirectDomRenderer.prototype.setAttribute=function(el,name,value){return el.setAttribute(name,value)},DirectDomRenderer.prototype.removeAttribute=function(el,name){el.removeAttribute(name)},DirectDomRenderer.prototype.setBindingDebugInfo=function(el,propertyName,propertyValue){},DirectDomRenderer.prototype.removeBindingDebugInfo=function(el,propertyName){},DirectDomRenderer.prototype.addClass=function(el,name){el.classList.add(name)},DirectDomRenderer.prototype.removeClass=function(el,name){el.classList.remove(name)},DirectDomRenderer.prototype.setStyle=function(el,style,value){el.style[style]=value},DirectDomRenderer.prototype.removeStyle=function(el,style){el.style[style]=""},DirectDomRenderer.prototype.setProperty=function(el,name,value){el[name]=value},DirectDomRenderer.prototype.setText=function(node,value){node.nodeValue=value},DirectDomRenderer.prototype.listen=function(target,eventName,callback){var renderTarget;switch(target){case"window":renderTarget=window;break;case"document":renderTarget=document;break;default:renderTarget=target}var closure=function(event){callback(event)===!1&&event.preventDefault()};return renderTarget.addEventListener(eventName,closure),function(){return renderTarget.removeEventListener(eventName,closure)}},DirectDomRenderer}(),EMPTY_V1_RENDER_COMPONENT_TYPE=new RenderComponentType("EMPTY","",0,ViewEncapsulation.None,[],{}),LegacyRendererAdapter=function(){function LegacyRendererAdapter(rootDelegate){this._delegate=rootDelegate.renderComponent(EMPTY_V1_RENDER_COMPONENT_TYPE)}return LegacyRendererAdapter.prototype.createElement=function(name,debugInfo){return this._delegate.createElement(null,name,debugInfo)},LegacyRendererAdapter.prototype.createComment=function(value,debugInfo){return this._delegate.createTemplateAnchor(null,debugInfo)},LegacyRendererAdapter.prototype.createText=function(value,debugInfo){return this._delegate.createText(null,value,debugInfo)},LegacyRendererAdapter.prototype.appendChild=function(parent,newChild){this._delegate.projectNodes(parent,[newChild])},LegacyRendererAdapter.prototype.insertBefore=function(parent,newChild,refChild){refChild?this._delegate.attachViewAfter(refChild.previousSibling,[newChild]):this.appendChild(parent,newChild)},LegacyRendererAdapter.prototype.removeChild=function(parent,oldChild){parent&&this._delegate.detachView([oldChild])},LegacyRendererAdapter.prototype.selectRootElement=function(selectorOrNode,debugInfo){return this._delegate.selectRootElement(selectorOrNode,debugInfo)},LegacyRendererAdapter.prototype.parentNode=function(node){return node.parentNode},LegacyRendererAdapter.prototype.nextSibling=function(node){return node.nextSibling},LegacyRendererAdapter.prototype.setAttribute=function(el,name,value){this._delegate.setElementAttribute(el,name,value)},LegacyRendererAdapter.prototype.removeAttribute=function(el,name){this._delegate.setElementAttribute(el,name,null)},LegacyRendererAdapter.prototype.setBindingDebugInfo=function(el,propertyName,propertyValue){this._delegate.setBindingDebugInfo(el,propertyName,propertyValue)},LegacyRendererAdapter.prototype.removeBindingDebugInfo=function(el,propertyName){this._delegate.setBindingDebugInfo(el,propertyName,null)},LegacyRendererAdapter.prototype.addClass=function(el,name){this._delegate.setElementClass(el,name,!0)},LegacyRendererAdapter.prototype.removeClass=function(el,name){this._delegate.setElementClass(el,name,!1)},LegacyRendererAdapter.prototype.setStyle=function(el,style,value){this._delegate.setElementStyle(el,style,value)},LegacyRendererAdapter.prototype.removeStyle=function(el,style){this._delegate.setElementStyle(el,style,null)},LegacyRendererAdapter.prototype.setProperty=function(el,name,value){this._delegate.setElementProperty(el,name,value)},LegacyRendererAdapter.prototype.setText=function(node,value){this._delegate.setText(node,value)},LegacyRendererAdapter.prototype.listen=function(target,eventName,callback){return"string"==typeof target?this._delegate.listenGlobal(target,eventName,callback):this._delegate.listen(target,eventName,callback)},LegacyRendererAdapter}(),NOOP=function(){},ViewAction={};ViewAction.CreateViewNodes=0,ViewAction.CheckNoChanges=1,ViewAction.CheckAndUpdate=2,ViewAction.Destroy=3,ViewAction[ViewAction.CreateViewNodes]="CreateViewNodes",ViewAction[ViewAction.CheckNoChanges]="CheckNoChanges",ViewAction[ViewAction.CheckAndUpdate]="CheckAndUpdate",ViewAction[ViewAction.Destroy]="Destroy";var QueryAction={};QueryAction.CheckAndUpdate=0,QueryAction.CheckNoChanges=1,QueryAction[QueryAction.CheckAndUpdate]="CheckAndUpdate",QueryAction[QueryAction.CheckNoChanges]="CheckNoChanges";var _currentAction,_currentView,_currentNodeIndex,initialized=!1,CAMEL_CASE_REGEXP$1=/([A-Z])/g,DebugRenderer=function(){function DebugRenderer(_delegate){this._delegate=_delegate}return DebugRenderer.prototype.createElement=function(name){return this._delegate.createElement(name,getCurrentDebugContext())},DebugRenderer.prototype.createComment=function(value){return this._delegate.createComment(value,getCurrentDebugContext())},DebugRenderer.prototype.createText=function(value){return this._delegate.createText(value,getCurrentDebugContext())},DebugRenderer.prototype.appendChild=function(parent,newChild){return this._delegate.appendChild(parent,newChild)},DebugRenderer.prototype.insertBefore=function(parent,newChild,refChild){return this._delegate.insertBefore(parent,newChild,refChild)},DebugRenderer.prototype.removeChild=function(parent,oldChild){return this._delegate.removeChild(parent,oldChild)},DebugRenderer.prototype.selectRootElement=function(selectorOrNode){return this._delegate.selectRootElement(selectorOrNode,getCurrentDebugContext())},DebugRenderer.prototype.parentNode=function(node){return this._delegate.parentNode(node)},DebugRenderer.prototype.nextSibling=function(node){return this._delegate.nextSibling(node)},DebugRenderer.prototype.setAttribute=function(el,name,value){return this._delegate.setAttribute(el,name,value)},DebugRenderer.prototype.removeAttribute=function(el,name){return this._delegate.removeAttribute(el,name)},DebugRenderer.prototype.setBindingDebugInfo=function(el,propertyName,propertyValue){this._delegate.setBindingDebugInfo(el,propertyName,propertyValue)},DebugRenderer.prototype.removeBindingDebugInfo=function(el,propertyName){this._delegate.removeBindingDebugInfo(el,propertyName)},DebugRenderer.prototype.addClass=function(el,name){return this._delegate.addClass(el,name)},DebugRenderer.prototype.removeClass=function(el,name){return this._delegate.removeClass(el,name)},DebugRenderer.prototype.setStyle=function(el,style,value){return this._delegate.setStyle(el,style,value)},DebugRenderer.prototype.removeStyle=function(el,style){return this._delegate.removeStyle(el,style)},DebugRenderer.prototype.setProperty=function(el,name,value){return this._delegate.setProperty(el,name,value)},DebugRenderer.prototype.setText=function(node,value){return this._delegate.setText(node,value)},DebugRenderer.prototype.listen=function(target,eventName,callback){return this._delegate.listen(target,eventName,callback)},DebugRenderer}(),DebugContext_=function(){function DebugContext_(view,nodeIndex){this.view=view,this.nodeIndex=nodeIndex,null==nodeIndex&&(this.nodeIndex=0),this.nodeDef=view.def.nodes[nodeIndex];for(var elIndex=nodeIndex,elView=view;null!=elIndex&&view.def.nodes[elIndex].type!==NodeType.Element;)elIndex=view.def.nodes[elIndex].parent;if(null==elIndex)for(;null==elIndex&&elView;)elIndex=viewParentDiIndex(elView),elView=elView.parent;if(this.elView=elView,elView){this.elDef=elView.def.nodes[elIndex];for(var i=this.elDef.index+1;i<=this.elDef.index+this.elDef.childCount;i++){var childDef=this.elView.def.nodes[i];if(childDef.flags&NodeFlags.HasComponent){this.compProviderIndex=i;break}i+=childDef.childCount}}else this.elDef=null}return Object.defineProperty(DebugContext_.prototype,"injector",{get:function(){return createInjector(this.elView,this.elDef.index)},enumerable:!0,configurable:!0}),Object.defineProperty(DebugContext_.prototype,"component",{get:function(){return null!=this.compProviderIndex?asProviderData(this.elView,this.compProviderIndex).instance:this.view.component},enumerable:!0,configurable:!0}),Object.defineProperty(DebugContext_.prototype,"context",{get:function(){return null!=this.compProviderIndex?asProviderData(this.elView,this.compProviderIndex).instance:this.view.context},enumerable:!0,configurable:!0}),Object.defineProperty(DebugContext_.prototype,"providerTokens",{get:function(){var tokens=[];if(this.elDef)for(var i=this.elDef.index+1;i<=this.elDef.index+this.elDef.childCount;i++){var childDef=this.elView.def.nodes[i];childDef.type===NodeType.Provider&&tokens.push(childDef.provider.token),i+=childDef.childCount}return tokens},enumerable:!0,configurable:!0}),Object.defineProperty(DebugContext_.prototype,"references",{get:function(){var references={};if(this.elDef){collectReferences(this.elView,this.elDef,references);for(var i=this.elDef.index+1;i<=this.elDef.index+this.elDef.childCount;i++){var childDef=this.elView.def.nodes[i];childDef.type===NodeType.Provider&&collectReferences(this.elView,childDef,references),i+=childDef.childCount}}return references},enumerable:!0,configurable:!0}),Object.defineProperty(DebugContext_.prototype,"source",{get:function(){return this.nodeDef.type===NodeType.Text?this.nodeDef.text.source:this.elDef.element.source},enumerable:!0,configurable:!0}),Object.defineProperty(DebugContext_.prototype,"componentRenderElement",{get:function(){var view=null!=this.compProviderIndex?asProviderData(this.elView,this.compProviderIndex).componentView:this.view,elData=findHostElement(view);return elData?elData.renderElement:void 0},enumerable:!0,configurable:!0}),Object.defineProperty(DebugContext_.prototype,"renderNode",{get:function(){return this.nodeDef.type===NodeType.Text?renderNode(this.view,this.nodeDef):renderNode(this.elView,this.elDef)},enumerable:!0,configurable:!0}),DebugContext_}(),viewEngine=Object.freeze({anchorDef:anchorDef,elementDef:elementDef,ngContentDef:ngContentDef,directiveDef:directiveDef,providerDef:providerDef,pureArrayDef:pureArrayDef,pureObjectDef:pureObjectDef,purePipeDef:purePipeDef,queryDef:queryDef,createComponentFactory:createComponentFactory,initServicesIfNeeded:initServicesIfNeeded,textDef:textDef,elementEventFullName:elementEventFullName,nodeValue:nodeValue,rootRenderNodes:rootRenderNodes,viewDef:viewDef,attachEmbeddedView:attachEmbeddedView,detachEmbeddedView:detachEmbeddedView,moveEmbeddedView:moveEmbeddedView,ArgumentType:ArgumentType,ViewFlags:ViewFlags,NodeType:NodeType,NodeFlags:NodeFlags,BindingType:BindingType,QueryValueType:QueryValueType,ProviderType:ProviderType,DepFlags:DepFlags,PureExpressionType:PureExpressionType,QueryBindingType:QueryBindingType,ViewState:ViewState,NodeData:NodeData,asTextData:asTextData,asElementData:asElementData,asProviderData:asProviderData,asPureExpressionData:asPureExpressionData,asQueryList:asQueryList,RenderDebugContext:RenderDebugContext,DebugContext:DebugContext,Services:Services}),ApplicationModule=function(){function ApplicationModule(){}return ApplicationModule}();ApplicationModule.decorators=[{type:NgModule,args:[{providers:[ApplicationRef_,{provide:ApplicationRef,useExisting:ApplicationRef_},ApplicationInitStatus,Compiler,APP_ID_RANDOM_PROVIDER,ViewUtils,AnimationQueue,{provide:IterableDiffers,useFactory:_iterableDiffersFactory},{provide:KeyValueDiffers,useFactory:_keyValueDiffersFactory},{provide:LOCALE_ID,useFactory:_localeFactory,deps:[[new Inject(LOCALE_ID),new Optional,new SkipSelf]]},{provide:APP_INITIALIZER,useValue:_initViewEngine,multi:!0}]}]}],ApplicationModule.ctorParameters=function(){return[]};/**
     * @license
     * Copyright Google Inc. All Rights Reserved.
     *
     * Use of this source code is governed by an MIT-style license that can be
