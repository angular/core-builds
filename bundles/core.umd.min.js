 ...
 * found in the LICENSE file at https://angular.io/license
 */
function _iterableDiffersFactory(){return defaultIterableDiffers}function _keyValueDiffersFactory(){return defaultKeyValueDiffers}function _localeFactory(locale){return locale||"en-US"}function isDOMParserAvailable(){try{return!!window.DOMParser}catch(e){return!1}}function _sanitizeUrl(url){return url=String(url),url.match(SAFE_URL_PATTERN)||url.match(DATA_URL_PATTERN)?url:(isDevMode()&&console.warn("WARNING: sanitizing unsafe URL value "+url+" (see http://g.co/ng/security#xss)"),"unsafe:"+url)}function sanitizeSrcset(srcset){return srcset=String(srcset),srcset.split(",").map(function(srcset){return _sanitizeUrl(srcset.trim())}).join(", ")}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 ...
 * found in the LICENSE file at https://angular.io/license
 */
function pipe(index,pipeDef,firstInstance){var tView=getTView();tView.firstTemplatePass&&(tView.data[index]=pipeDef,null!=pipeDef.onDestroy&&(tView.destroyHooks||(tView.destroyHooks=[])).push(index,pipeDef.onDestroy));var pipeInstance=pipeDef.pure&&firstInstance?firstInstance:pipeDef.n();return store(index,pipeInstance),pipeInstance}function pipeBind1(index,v1){var pipeInstance=load(index);return isPure(index)?pureFunction1(pipeInstance.transform,v1,pipeInstance):pipeInstance.transform(v1)}function pipeBind2(index,v1,v2){var pipeInstance=load(index);return isPure(index)?pureFunction2(pipeInstance.transform,v1,v2,pipeInstance):pipeInstance.transform(v1,v2)}function pipeBind3(index,v1,v2,v3){var pipeInstance=load(index);return isPure(index)?pureFunction3(pipeInstance.transform.bind(pipeInstance),v1,v2,v3):pipeInstance.transform(v1,v2,v3)}function pipeBind4(index,v1,v2,v3,v4){var pipeInstance=load(index);return isPure(index)?pureFunction4(pipeInstance.transform,v1,v2,v3,v4,pipeInstance):pipeInstance.transform(v1,v2,v3,v4)}function pipeBindV(index,values){var pipeInstance=load(index);return isPure(index)?pureFunctionV(pipeInstance.transform,values,pipeInstance):pipeInstance.transform.apply(pipeInstance,values)}function isPure(index){return getTView().data[index].pure}function getIdxOfMatchingSelector(tNode,selector){var localNames=tNode.localNames;if(localNames)for(var i=0;i<localNames.length;i+=2)if(localNames[i]===selector)return localNames[i+1];return null}function geIdxOfMatchingDirective(node,type){for(var tData=node.view.tView.data,flags=node.flags,i=flags>>12,ii=i+((4092&flags)>>2);i<ii;i++){var def=tData[i];if(def.diPublic&&def.type===type)return i}return null}function readFromNodeInjector(nodeInjector,node,read,directiveIdx){if(read instanceof ReadFromInjectorFn)return read.read(nodeInjector,node,directiveIdx);var matchingIdx=geIdxOfMatchingDirective(node,read);return null!==matchingIdx?node.view.data[matchingIdx]:null}function add(query,node){for(var nodeInjector=getOrCreateNodeInjectorForNode(node);query;){var predicate=query.predicate,type=predicate.type;if(type){var directiveIdx=geIdxOfMatchingDirective(node,type);if(null!==directiveIdx){var result=readFromNodeInjector(nodeInjector,node,predicate.read||type,directiveIdx);null!==result&&addMatch(query,result)}}else for(var selector=predicate.selector,i=0;i<selector.length;i++){ngDevMode&&assertNotNull(node.tNode,"node.tNode");var directiveIdx=getIdxOfMatchingSelector(node.tNode,selector[i]);if(null!==directiveIdx){ngDevMode&&assertNotNull(predicate.read,"the node should have a predicate");var result=readFromNodeInjector(nodeInjector,node,predicate.read,directiveIdx);null!==result&&addMatch(query,result)}}query=query.next}}function addMatch(query,matchingValue){query.values.push(matchingValue),query.list.setDirty()}function createPredicate(predicate,read){var isArray=Array.isArray(predicate);return{type:isArray?null:predicate,selector:isArray?predicate:null,read:read}}function createQuery$1(previous,queryList,predicate,read){return{next:previous,list:queryList,predicate:createPredicate(predicate,read),values:queryList._valuesTree}}function query(memoryIndex,predicate,descend,read){ngDevMode&&assertPreviousIsParent();var queryList=new QueryList$1;return getCurrentQueries(LQueries_).track(queryList,predicate,descend,read),null!=memoryIndex&&store(memoryIndex,queryList),queryList}function queryRefresh(queryList){var queryListImpl=queryList;return!!queryList.dirty&&(queryList.reset(queryListImpl._valuesTree),queryList.notifyOnChanges(),!0)}function sanitizeHtml(unsafeHtml){return unsafeHtml instanceof String&&"Html"===unsafeHtml[BRAND]?unsafeHtml.toString():_sanitizeHtml(document,stringify$1(unsafeHtml))}function sanitizeStyle(unsafeStyle){return unsafeStyle instanceof String&&"Style"===unsafeStyle[BRAND]?unsafeStyle.toString():_sanitizeStyle(stringify$1(unsafeStyle))}function sanitizeUrl(unsafeUrl){return unsafeUrl instanceof String&&"Url"===unsafeUrl[BRAND]?unsafeUrl.toString():_sanitizeUrl(stringify$1(unsafeUrl))}function sanitizeResourceUrl(unsafeResourceUrl){if(unsafeResourceUrl instanceof String&&"ResourceUrl"===unsafeResourceUrl[BRAND])return unsafeResourceUrl.toString();throw new Error("unsafe value used in a resource URL context (see http://g.co/ng/security#xss)")}function bypassSanitizationTrustHtml(trustedHtml){return bypassSanitizationTrustString(trustedHtml,"Html")}function bypassSanitizationTrustStyle(trustedStyle){return bypassSanitizationTrustString(trustedStyle,"Style")}function bypassSanitizationTrustScript(trustedScript){return bypassSanitizationTrustString(trustedScript,"Script")}function bypassSanitizationTrustUrl(trustedUrl){return bypassSanitizationTrustString(trustedUrl,"Url")}function bypassSanitizationTrustResourceUrl(trustedResourceUrl){return bypassSanitizationTrustString(trustedResourceUrl,"ResourceUrl")}function bypassSanitizationTrustString(trustedString,mode){var trusted=new String(trustedString);return trusted[BRAND]=mode,trusted}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
