...
const currentFileUrl = isCommonJS ? null : (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('checker-YrM3rMH-.cjs', document.baseURI).href));
...
class SymbolBuilder {
    ...
    getSymbol(node) {
        ...
        else if (node instanceof compiler.Directive) {
    ...
    getDirectiveMeta(host, directiveDeclaration) {
        ...
        if (!(host instanceof compiler.Directive)) {
            const firstChild = host.children[0];
            if (firstChild instanceof compiler.Element) {
                const isMicrosyntaxTemplate = host instanceof compiler.Template &&
