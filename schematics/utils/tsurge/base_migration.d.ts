...
export type MigrationStats<T> = T extends TsurgeBaseMigration<unknown, unknown, infer Stats> ? Stats : never;
/**
 * @private
 *
 ...
 */
export declare abstract class TsurgeBaseMigration<UnitAnalysisMetadata, CombinedGlobalMetadata, Stats = unknown> {
    /**
     ...
     */
    createProgram(tsconfigAbsPath: string, fs: FileSystem, optionsOverride?: NgCompilerOptions): ProgramInfo;
     ...
     */
    abstract combine(unitA: UnitAnalysisMetadata, unitB: UnitAnalysisMetadata): Promise<Serializable<UnitAnalysisMetadata>>;
    /**
     * Converts combined compilation into global metadata result that
     * is then available for migrate and stats stages.
     */
    abstract globalMeta(combinedData: UnitAnalysisMetadata): Promise<Serializable<CombinedGlobalMetadata>>;
