...
function renderEmbeddedTemplate(viewNode, tView, context, renderer, queries) {
    ...
    try {
        ...
        previousOrParentNode = null;
        if (viewNode == null) {
            const lView = createLView(renderer, tView, context, 2 /* CheckAlways */, getCurrentSanitizer());
            if (queries) {
...
function hostElement(tag, rNode, def, sanitizer) {
    ...
    const node = createLNode(0, 3 /* Element */, rNode, null, null, createLView(renderer, getOrCreateTView(def.template, def.directiveDefs, def.pipeDefs), null, def.onPush ? 4 /* Dirty */ : 2 /* CheckAlways */, sanitizer));
    if (firstTemplatePass) {
 ...
 */
function storeCleanupWithContext(view = currentView, context, cleanupFn) {
...
function addComponentLogic(index, instance, def) {
    ...
    const hostView = addToViewTree(currentView, previousOrParentNode.tNode.index, createLView(rendererFactory.createRenderer(previousOrParentNode.native, def.rendererType), tView, null, def.onPush ? 4 /* Dirty */ : 2 /* CheckAlways */, getCurrentSanitizer()));
...
function embeddedViewStart(viewBlockId) {
    ...
    if (viewNode) {
        ...
        const newView = createLView(renderer, getOrCreateEmbeddedTView(viewBlockId, container), null, 2 /* CheckAlways */, getCurrentSanitizer());
