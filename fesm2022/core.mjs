...
import { SIGNAL, producerAccessed, signalSetFn, consumerDestroy, SIGNAL_NODE, consumerPollProducersForChange, consumerBeforeComputation, consumerAfterComputation } from './_effect-chunk.mjs';
...
function createComponent(component, options) {
  ...
  const componentRef = factory.create(elementInjector, options.projectableNodes, options.hostElement, options.environmentInjector, options.directives, options.bindings);
  if (options.onError) {
    componentRef.hostView._lView[ON_ERROR] = options.onError;
  }
  return componentRef;
}
